...
(function (global, factory) {
	...
	function observe (value, asRootData) {
		...
		if ( hasOwn (value, '__ob__') && value.__ob__ instanceof
				Observer ) {ob = value.__ob__} else if ( shouldObserve &&
				!isServerRendering () && (Array.isArray (value) || isPlainObject (value)) &&
	...
	function validateProp (key, propOptions, propsData, vm) {
		...
		if ( booleanIndex > -1 ) {
			if ( absent && !hasOwn (prop, 'default') ) {value = false} else if ( value === '' || value ===
					hyphenate (key) ) {
	...
	function normalizeScopedSlots (slots, normalSlots, prevSlots) {
		...
		if ( !slots ) {res = {}} else if ( slots._normalized ) {return slots._normalized} else if ( isStable &&
				prevSlots &&
	...
	function normalizeScopedSlot (normalSlots, key, fn) {
		...
		if ( fn.proxy ) {
			Object.defineProperty (normalSlots, key, { get: normalized, enumerable: true, configurable: true })
		}
		return normalized
	}
	...
	function FunctionalRenderContext (data, props, children, parent, Ctor) {
		...
		Object.defineProperty (this, 'scopedSlots',
				({
					enumerable: true,
					get: function get () {return normalizeScopedSlots (data.scopedSlots, this.slots ())},
				}))
		if ( isCompiled ) {
	...
	if ( inBrowser && window.performance && typeof performance.now === 'function' &&
			...
			performance.now () ) {getNow = function () {return performance.now ()}}
	...
	function matches (pattern, name) {
		if ( Array.isArray (pattern) ) {return pattern.indexOf (name) > -1} else if ( typeof pattern ===
				'string' ) {return pattern.split (',').indexOf (name) > -1} else if ( isRegExp (
				pattern) ) {return pattern.test (name)}
		return false
	}
	...
	function registerRef (vnode, isRemoval) {
		...
		var refs = vm.$refs
		if ( isRemoval ) {
			if ( Array.isArray (refs[key]) ) {remove (refs[key], ref)} else if ( refs[key] ===
					ref ) {refs[key] = undefined}
		} else {
			if ( vnode.data.refInFor ) {
				if ( !Array.isArray (refs[key]) ) {refs[key] = [ref]} else if ( refs[key].indexOf (ref) <
						0 ) {refs[key].push (ref)}
			} else {refs[key] = ref}
		}
	}
		...
		return function patch (oldVnode, vnode, hydrating, removeOnly) {
			...
			if ( isUndef (oldVnode) ) {
				...
				if ( !isRealElement && sameVnode (oldVnode, vnode) ) {
					...
					createElm (vnode, insertedVnodeQueue, oldElm._leaveCb ? null : parentElm,
	...
	function parseFilters (exp) {
		...
		for ( i = 0; i < exp.length; i++ ) {
			...
			if ( inSingle ) {if ( c === 0x27 && prev !== 0x5C ) {inSingle = false}} else if ( inDouble ) {
				...
				if ( c === 0x60 && prev !== 0x5C ) {inTemplateString = false}
					...
					exp.charCodeAt (i + 1) !== 0x7C && exp.charCodeAt (i - 1) !== 0x7C && !curly && !square &&
					!paren ) {
		...
		if ( expression === undefined ) {expression = exp.slice (0, i).trim ()} else if ( lastFilterIndex !==
				0 ) {pushFilter ()}
	...
	function isStatic (node) {
		...
		return !!(node.pre ||
				( !node.hasBindings && !node.if && !node.for && !isBuiltInTag (node.tag) &&
	...
	function genElement (el, state) {
		...
		} else if ( el.if && !el.ifProcessed ) {return genIf (el, state)} else if ( el.tag === 'template' &&
				!el.slotTarget &&
	...
	function genScopedSlot (el, state) {
		...
		var fn = 'function(' + slotScope + '){' +
				'return ' + (el.tag === 'template' ? el.if && isLegacySyntax ? ('(' + (el.if) + ')?' +
						(genChildren (el, state) || 'undefined') + ':undefined') : genChildren (el, state) ||
						'undefined' : genElement (el,
						state)) + '}'
	...
	function getNormalizationType (children, maybeComponent) {
		...
		for ( var i = 0; i < children.length; i++ ) {
			...
			if ( maybeComponent (el) ||
					(el.ifConditions &&
							el.ifConditions.some (function (c) {return maybeComponent (c.block)})) ) {res = 1}
		}
		return res
	}
	...
	function createCompileToFunctionFn (compile) {
		...
		return function compileToFunctions (template, options, vm) {
			...
			res.staticRenderFns = compiled.staticRenderFns.map (
					function (code) {return createFunction (code, fnGenErrors)})
			{
