...
(function (global, factory) {
  ...
  var camelizeRE = /-(\w)/g
  var camelize = cached(
    function (str) {return str.replace(camelizeRE, function (_, c) {return c ? c.toUpperCase() : ''})})
  ...
  function genStaticKeys (modules) {
    return modules.reduce(function (keys, m) {return keys.concat(m.staticKeys || [])}, []).join(',')
  }
    ...
    if (isObjectA && isObjectB) {
      try {
        ...
        if (isArrayA && isArrayB) {
          return a.length === b.length && a.every(function (e, i) {return looseEqual(e, b[i])})
        } else if (a instanceof Date && b instanceof Date) {return a.getTime() === b.getTime()} else if (!isArrayA &&
          !isArrayB) {
  ...
  if (inBrowser) {
    try {
      var opts = {}
      Object.defineProperty(opts, 'passive', ({ get: function get () {supportsPassive = true} }))
  ...
  var _Set
  if (typeof Set !== 'undefined' && isNative(Set)) {_Set = Set} else {
    _Set = (function () {
      function Set () {this.set = Object.create(null)}
      
      Set.prototype.has = function has (key) {return this.set[key] === true}
      Set.prototype.add = function add (key) {this.set[key] = true}
  ...
  var formatComponentName = (noop)
  {
    ...
    var classify = function (str) {
      return str.replace(classifyRE, function (c) {return c.toUpperCase()}).
        replace(/[-_]/g, '')
    }
    warn = function (msg, vm) {
      var trace = vm ? generateComponentTrace(vm) : ''
      if (config.warnHandler) {config.warnHandler.call(null, msg, vm, trace)} else if (hasConsole &&
        (!config.silent)) {console.error(('[Vue warn]: ' + msg + trace))}
    }
    tip = function (msg, vm) {
  ...
  Dep.prototype.notify = function notify () {
    ...
    if (!config.async) {subs.sort(function (a, b) {return a.id - b.id})}
  ...
  prototypeAccessors.child.get = function () {return this.componentInstance}
  ...
  function observe (value, asRootData) {
    ...
    if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {ob = value.__ob__} else if (shouldObserve &&
      ...
      !value._isVue) {ob = new Observer(value)}
  ...
  function mergeData (to, from) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      toVal = to[key]
      fromVal = from[key]
      if (!hasOwn(to, key)) {set(to, key, fromVal)} else if (toVal !== fromVal && isPlainObject(toVal) &&
        isPlainObject(fromVal)) {mergeData(toVal, fromVal)}
    }
    return to
  }
  ...
  strats.props = strats.methods = strats.inject = strats.computed = function (parentVal, childVal, vm, key) {
    if (childVal && 'development' !== 'production') {assertObjectType(key, childVal, vm)}
    if (!parentVal) {return childVal}
  ...
  function normalizeInject (options, vm) {
    ...
    if (Array.isArray(inject)) {
      for (var i = 0; i < inject.length; i++) {normalized[inject[i]] = { from: inject[i] }}
  ...
  function normalizeDirectives (options) {
    ...
    if (dirs) {
      for (var key in dirs) {
        var def$$1 = dirs[key]
        if (typeof def$$1 === 'function') {dirs[key] = { bind: def$$1, update: def$$1 }}
      }
    }
  }
  ...
  function validateProp (key, propOptions, propsData, vm) {
    ...
    if (booleanIndex > -1) {
      if (absent && !hasOwn(prop, 'default')) {value = false} else if (value === '' || value === hyphenate(key)) {
  ...
  function getPropDefaultValue (vm, prop, key) {
    ...
    if (vm && vm.$options.propsData && vm.$options.propsData[key] === undefined && vm._props[key] !==
      undefined) {return vm._props[key]}
  ...
  function assertType (value, type) {
    ...
    } else if (expectedType === 'Object') {valid = isPlainObject(value)} else if (expectedType ===
      'Array') {valid = Array.isArray(value)} else {valid = value instanceof type}
  ...
  function styleValue (value, type) {
    if (type === 'String') {return ('"' + value + '"')} else if (type === 'Number') {
      return ('' + (Number(value)))
    } else {return ('' + value)}
  }
  ...
  function globalHandleError (err, vm, info) {
    if (config.errorHandler) {
      try {return config.errorHandler.call(null, err, vm, info)} catch (e) {
  ...
  var initProxy
  {
    ...
    var warnNonPresent = function (target, key) {
      warn('属性或方法 "' + key + '" 未在实例上定义 但 ' +
        '在渲染期间引用. 确保此属性是反应性的, ' +
        '无论是在数据选项中，还是对于基于类的组件, 通过 ' +
        '初始化属性。 ' +
        '看: https://vuejs.org/v2/guide/reactivity.html#Declaring-Reactive-Properties.', target)
    }
    var warnReservedPrefix = function (target, key) {
      warn('属性 "' + key + '" 必须使用“$data”访问.' + key + '" 因为 ' +
  ...
  function mergeVNodeHook (def, hookKey, hook) {
    ...
    if (isUndef(oldHook)) {invoker = createFnInvoker([wrappedHook])} else {
  ...
  function normalizeArrayChildren (children, nestedIndex) {
    ...
    for (i = 0; i < children.length; i++) {
      ...
      if (Array.isArray(c)) {
        ...
        if (isTextNode(c) && isTextNode(last)) {res[lastIndex] = createTextVNode(last.text + c.text)} else {
  ...
  function resolveSlots (children, context) {
    ...
    for (var i = 0, l = children.length; i < l; i++) {
      ...
      if ((child.context === context || child.fnContext === context) && data && data.slot != null) {
        ...
        if (child.tag === 'template') {slot.push.apply(slot, child.children || [])} else {slot.push(child)}
  ...
  function normalizeScopedSlots (slots, normalSlots, prevSlots) {
    ...
    if (!slots) {res = {}} else if (slots._normalized) {return slots._normalized} else if (isStable && prevSlots &&
      prevSlots !== emptyObject && key === prevSlots.$key && !hasNormalSlots &&
      !prevSlots.$hasNormal) {return prevSlots} else {
      res = {}
  ...
  function renderSlot (name, fallback, props, bindObject) {
    ...
    if (target) {return this.$createElement('template', { slot: target }, nodes)} else {return nodes}
  }
  ...
  function bindObjectProps (data, tag, value, asProp, isSync) {
    if (value) {
      if (!isObject(value)) {warn('v-bind without argument expects an Object or Array value', this)} else {
        if (Array.isArray(value)) {value = toObject(value)}
        var hash
        var loop = function (key) {
          if (key === 'class' || key === 'style' || isReservedAttribute(key)) {hash = data} else {
          ...
          if (!(camelizedKey in hash) && !(hyphenatedKey in hash)) {
            ...
            if (isSync) {
              ...
              on[('update:' + key)] = function ($event) {value[key] = $event}
            }
          }
        }
  ...
  function resolveScopedSlots (fns, res, hasDynamicKeys, contentHashKey) {
    ...
    for (var i = 0; i < fns.length; i++) {
      var slot = fns[i]
      if (Array.isArray(slot)) {resolveScopedSlots(slot, res, hasDynamicKeys)} else if (slot) {
  ...
  function bindDynamicKeys (baseObj, values) {
    for (var i = 0; i < values.length; i += 2) {
      var key = values[i]
      if (typeof key === 'string' && key) {baseObj[values[i]] = values[i + 1]} else if (key !== '' && key !==
        null) {warn(('Invalid value for dynamic directive argument (expected string or null): ' + key), this)}
    }
    return baseObj
  }
  ...
  function FunctionalRenderContext (data, props, children, parent, Ctor) {
    ...
    Object.defineProperty(this, 'scopedSlots',
      ({ enumerable: true, get: function get () {return normalizeScopedSlots(data.scopedSlots, this.slots())} }))
    if (isCompiled) {
    ...
    } else {this._c = function (a, b, c, d) {return createElement(contextVm, a, b, c, d, needNormalization)}}
  }
  ...
  function createComponent (Ctor, data, context, children, tag) {
    ...
    var vnode = new VNode(('vue-component-' + (Ctor.cid) + (name ? ('-' + name) : '')), data, undefined, undefined,
      undefined, context, { Ctor: Ctor, propsData: propsData, listeners: listeners, tag: tag, children: children },
      asyncFactory)
    return vnode
  }
  ...
  function transformModel (options, data) {
    ...
    if (isDef(existing)) {
      if (Array.isArray(existing) ? existing.indexOf(callback) === -1 : existing !==
        callback) {on[event] = [callback].concat(existing)}
  ...
  function _createElement (context, tag, data, children, normalizationType) {
    ...
    if (normalizationType === ALWAYS_NORMALIZE) {children = normalizeChildren(children)} else if (normalizationType ===
      SIMPLE_NORMALIZE) {children = simpleNormalizeChildren(children)}
    var vnode, ns
    ...
    if (Array.isArray(vnode)) {return vnode} else if (isDef(vnode)) {
  ...
  function initRender (vm) {
    ...
    var parentData = parentVnode && parentVnode.data
    {
      defineReactive$$1(vm, '$attrs', parentData && parentData.attrs || emptyObject,
        function () {!isUpdatingChildComponent && warn('$attrs is readonly.', vm)}, true)
      defineReactive$$1(vm, '$listeners', options._parentListeners || emptyObject,
        function () {!isUpdatingChildComponent && warn('$listeners is readonly.', vm)}, true)
    }
  }
  ...
  function resolveAsyncComponent (factory, baseCtor) {
    ...
    if (owner && !isDef(factory.owners)) {
      ...
      if (isObject(res)) {
        if (isPromise(res)) {if (isUndef(factory.resolved)) {res.then(resolve, reject)}} else if (isPromise(
          res.component)) {
          ...
          if (isDef(res.timeout)) {
            setTimeout(function () {if (isUndef(factory.resolved)) {reject('timeout (' + (res.timeout) + 'ms)')}},
              res.timeout)
          }
        }
      }
  ...
  function setActiveInstance (vm) {
    ...
    activeInstance = vm
    return function () {activeInstance = prevActiveInstance}
  }
  ...
  function mountComponent (vm, el, hydrating) {
    ...
    new Watcher(vm, updateComponent, noop,
      { before: function before () {if (vm._isMounted && !vm._isDestroyed) {callHook(vm, 'beforeUpdate')}} }, true)
    hydrating = false
    if (vm.$vnode == null) {
      vm._isMounted = true
      callHook(vm, 'mounted')
  ...
  if (inBrowser && window.performance && typeof performance.now === 'function' &&
    document.createEvent('Event').timeStamp <= performance.now()) {getNow = function () {return performance.now()}}
  ...
  Watcher.prototype.get = function get () {
    ...
    var value
    var vm = this.vm
    try {value = this.getter.call(vm, vm)} catch (e) {
      if (this.user) {
  ...
  Watcher.prototype.update = function update () {
    if (this.lazy) {this.dirty = true} else if (this.sync) {this.run()} else {
      queueWatcher(this)
    }
  }
  Watcher.prototype.run = function run () {
    if (this.active) {
      ...
      if (value !== this.value || isObject(value) || this.deep) {
        ...
        this.value = value
        if (this.user) {
          try {this.cb.call(this.vm, value, oldValue)} catch (e) {
  ...
  function getData (data, vm) {
    pushTarget()
    try {return data.call(vm, vm)} catch (e) {
  ...
  function initComputed (vm, computed) {
    ...
    for (var key in computed) {
      ...
      if (!(key in vm)) {defineComputed(vm, key, userDef)} else {
        if (key in vm.$data) {
  ...
  function stateMixin (Vue) {
    ...
    Vue.prototype.$watch = function (expOrFn, cb, options) {
      ...
      if (options.immediate) {
        try {cb.call(vm, watcher.value)} catch (error) {
  ...
  function initMixin (Vue) {
    Vue.prototype._init = function (options) {
      ...
      vm._isVue = true
      if (options && options._isComponent) {initInternalComponent(vm, options)} else {
  ...
  function initUse (Vue) {
    Vue.use = function (plugin) {
      ...
      var args = toArray(arguments, 1)
      args.unshift(this)
      if (typeof plugin.install === 'function') {plugin.install.apply(plugin, args)} else if (typeof plugin ===
        'function') {plugin.apply(null, args)}
    ...
    ASSET_TYPES.forEach(function (type) {
      Vue[type] = function (id, definition) {
        if (!definition) {return this.options[type + 's'][id]} else {
          if (type === 'component') {validateComponentName(id)}
  ...
  function matches (pattern, name) {
    if (Array.isArray(pattern)) {return pattern.indexOf(name) > -1} else if (typeof pattern ===
      'string') {return pattern.split(',').indexOf(name) > -1} else if (isRegExp(pattern)) {return pattern.test(name)}
    return false
  }
  ...
  var KeepAlive = {
    ...
    render: function render () {
      ...
      if (componentOptions) {
        ...
        var keys = ref$1.keys
        var key = vnode.key == null ? componentOptions.Ctor.cid +
  ...
  function initGlobalAPI (Vue) {
    ...
    {configDef.set = function () {warn('Do not replace the Vue.config object, set individual fields instead.')}}
    Object.defineProperty(Vue, 'config', configDef)
    Vue.util = { warn: warn, extend: extend, mergeOptions: mergeOptions, defineReactive: defineReactive$$1 }
    Vue.set = set
    Vue.delete = del
  ...
  function isUnknownElement (tag) {
    ...
    if (tag.indexOf('-') > -1) {
      return (unknownElementCache[tag] = (el.constructor === window.HTMLUnknownElement || el.constructor ===
        window.HTMLElement))
  ...
  var ref = {
    create: function create (_, vnode) {registerRef(vnode)},
    ...
    destroy: function destroy (vnode) {registerRef(vnode, true)},
  }
  ...
  function registerRef (vnode, isRemoval) {
    ...
    var refs = vm.$refs
    if (isRemoval) {
      if (Array.isArray(refs[key])) {remove(refs[key], ref)} else if (refs[key] === ref) {refs[key] = undefined}
    } else {
      if (vnode.data.refInFor) {
        if (!Array.isArray(refs[key])) {refs[key] = [ref]} else if (refs[key].indexOf(ref) < 0) {refs[key].push(ref)}
      } else {refs[key] = ref}
    }
  }
  ...
  function createPatchFunction (backend) {
    ...
    function createRmCb (childElm, listeners) {
      function remove$$1 () {if (--remove$$1.listeners === 0) {removeNode(childElm)}}
    ...
    function isUnknownElement$$1 (vnode, inVPre) {
      return (!inVPre && !vnode.ns && !(config.ignoredElements.length && config.ignoredElements.some(
          function (ignore) {return isRegExp(ignore) ? ignore.test(vnode.tag) : ignore === vnode.tag})) &&
    ...
    function removeAndInvokeRemoveHook (vnode, rm) {
      if (isDef(rm) || isDef(vnode.data)) {
        ...
        if (isDef(rm)) {rm.listeners += listeners} else {rm = createRmCb(vnode.elm, listeners)}
    ...
    function updateChildren (parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {oldStartVnode = oldCh[++oldStartIdx]} else if (isUndef(
          oldEndVnode)) {oldEndVnode = oldCh[--oldEndIdx]} else if (sameVnode(oldStartVnode, newStartVnode)) {
    ...
    function patchVnode (oldVnode, vnode, insertedVnodeQueue, ownerArray, index, removeOnly) {
      ...
      if (isUndef(vnode.text)) {
        ...
        } else if (isDef(oldCh)) {removeVnodes(elm, oldCh, 0, oldCh.length - 1)} else if (isDef(
          oldVnode.text)) {nodeOps.setTextContent(elm, '')}
      } else if (oldVnode.text !== vnode.text) {nodeOps.setTextContent(elm, vnode.text)}
      if (isDef(data)) {if (isDef(i = data.hook) && isDef(i = i.postpatch)) {i(oldVnode, vnode)}}
    }
    
    function invokeInsertHook (vnode, queue, initial) {
      if (isTrue(initial) && isDef(vnode.parent)) {vnode.parent.data.pendingInsert = queue} else {
    ...
    return function patch (oldVnode, vnode, hydrating, removeOnly) {
      ...
      if (isUndef(oldVnode)) {
        ...
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          ...
          if (isDef(parentElm)) {removeVnodes(parentElm, [oldVnode], 0, 0)} else if (isDef(
            oldVnode.tag)) {invokeDestroyHook(oldVnode)}
        }
      }
  ...
  function _update (oldVnode, vnode) {
    ...
    if (dirsWithInsert.length) {
      ...
      if (isCreate) {mergeVNodeHook(vnode, 'insert', callInsert)} else {callInsert()}
    }
  ...
  function callHook$1 (dir, hook, vnode, oldVnode, isDestroy) {
    ...
    if (fn) {
      try {fn(vnode.elm, dir, vnode, oldVnode, isDestroy)} catch (e) {
  ...
  function setAttr (el, key, value) {
    ...
    } else if (isEnumeratedAttr(key)) {el.setAttribute(key, convertEnumeratedValue(key, value))} else if (isXlink(
      key)) {
      if (isFalsyAttrValue(value)) {el.removeAttributeNS(xlinkNS, getXlinkProp(key))} else {
  ...
  function updateClass (oldVnode, vnode) {
    ...
    if (isUndef(data.staticClass) && isUndef(data.class) &&
      (isUndef(oldData) || (isUndef(oldData.staticClass) && isUndef(oldData.class)))) {return}
  ...
  function parseFilters (exp) {
    ...
    for (i = 0; i < exp.length; i++) {
      prev = c
      c = exp.charCodeAt(i)
      if (inSingle) {if (c === 0x27 && prev !== 0x5C) {inSingle = false}} else if (inDouble) {
        if (c === 0x22 && prev !== 0x5C) {inDouble = false}
      } else if (inTemplateString) {
        if (c === 0x60 && prev !== 0x5C) {inTemplateString = false}
      } else if (inRegex) {if (c === 0x2f && prev !== 0x5C) {inRegex = false}} else if (c === 0x7C &&
  ...
  function wrapFilter (exp, filter) {
    ...
    if (i < 0) {return ('_f("' + filter + '")(' + exp + ')')} else {
  ...
  function pluckModuleFunction (modules, key) {
    return modules ? modules.map(function (m) {return m[key]}).
  ...
  function getBindingAttr (el, name, getStatic) {
    ...
    if (dynamicValue != null) {return parseFilters(dynamicValue)} else if (getStatic !== false) {
  ...
  function genAssignmentCode (value, assignment) {
    ...
    if (res.key === null) {return (value + '=' + assignment)} else {
  ...
  function parseModel (val) {
    ...
    while (!eof()) {
      chr = next()
      if (isStringStart(chr)) {parseString(chr)} else if (chr === 0x5B) {parseBracket(chr)}
    }
  ...
  function model (el, dir, _warn) {
    ...
    if (el.component) {
      ...
      return false
    } else if (tag === 'select') {genSelect(el, value, modifiers)} else if (tag === 'input' && type ===
      'checkbox') {genCheckboxModel(el, value, modifiers)} else if (tag === 'input' && type ===
      'radio') {genRadioModel(el, value, modifiers)} else if (tag === 'input' || tag === 'textarea') {
  ...
  function add$1 (name, handler, capture, passive) {
    if (useMicrotaskFix) {
      ...
      handler = original._wrapper = function (e) {
        if (e.target === e.currentTarget || e.timeStamp >= attachedTimestamp || e.timeStamp <= 0 ||
  ...
  function updateDOMProps (oldVnode, vnode) {
    ...
    for (key in props) {
      ...
      } else if (cur !== oldProps[key]) {try {elm[key] = cur} catch (e) {}}
    }
  }
  ...
  function getStyle (vnode, checkChild) {
    ...
    while ((parentNode = parentNode.parent)) {
      if (parentNode.data && (styleData = normalizeStyleData(parentNode.data))) {extend(res, styleData)}
    }
    return res
  }
  ...
  function whenTransitionEnds (el, expectedType, cb) {
    ...
    var onEnd = function (e) {if (e.target === el) {if (++ended >= propCount) {end()}}}
    setTimeout(function () {if (ended < propCount) {end()}}, timeout + 1)
  ...
  var transition = inBrowser ? {
    create: _enter,
    activate: _enter,
    remove: function remove$$1 (vnode, rm) {if (vnode.data.show !== true) {leave(vnode, rm)} else {rm()}},
  } : {}
    ...
    inserted: function inserted (el, binding, vnode, oldVnode) {
      if (vnode.tag === 'select') {
        if (oldVnode.elm && !oldVnode.elm._vOptions) {
          mergeVNodeHook(vnode, 'postpatch', function () {directive.componentUpdated(el, binding, vnode)})
  ...
  function setSelected (el, binding, vm) {
    ...
    if (isIE || isEdge) {setTimeout(function () {actuallySetSelected(el, binding, vm)}, 0)}
  }
  ...
  function locateNode (vnode) {
    return vnode.componentInstance && (!vnode.data || !vnode.data.transition) ? locateNode(
  ...
  var show = {
    bind: function bind (el, ref, vnode) {
      ...
      if (value && transition$$1) {
        ...
        enter(vnode, function () {el.style.display = originalDisplay})
    ...
    update: function update (el, ref, vnode) {
      ...
      if (transition$$1) {
        vnode.data.show = true
        if (value) {enter(vnode, function () {el.style.display = el.__vOriginalDisplay})} else {
          leave(vnode, function () {el.style.display = 'none'})
        }
      } else {el.style.display = value ? el.__vOriginalDisplay : 'none'}
    },
    unbind: function unbind (el, binding, vnode, oldVnode,
      isDestroy) {if (!isDestroy) {el.style.display = el.__vOriginalDisplay}},
  }
  ...
  var Transition = {
    name: 'transition', props: transitionProps, abstract: true, render: function render (h) {
      var this$1 = this
  ...
  delete props.mode
  var TransitionGroup = {
    props: props, beforeMount: function beforeMount () {
      var this$1 = this
      ...
      hasMove: function hasMove (el, moveClass) {
        ...
        if (el._transitionClasses) {el._transitionClasses.forEach(function (cls) {removeClass(clone, cls)})}
  ...
  if (inBrowser) {
    setTimeout(function () {
      if (config.devtools) {
        if (devtools) {devtools.emit('init', Vue)} else {
  ...
  function parse (template, options) {
    ...
    function closeElement (element) {
      ...
      if (currentParent && !element.forbidden) {
        if (element.elseif || element.else) {processIfConditions(element, currentParent)} else {
    ...
    parseHTML(template, {
      ...
      start: function start (tag, attrs, unary, start$1, end) {
        ...
        if (inVPre) {processRawAttrs(element)} else if (!element.processed) {
          processFor(element)
          processIf(element)
          processOnce(element)
        }
      ...
      chars: function chars (text, start, end) {
        ...
        } else if (!children.length) {text = ''} else if (whitespaceOption) {
          if (whitespaceOption === 'condense') {text = lineBreakRE.test(text) ? '' : ' '} else {text = ' '}
        } else {text = preserveWhitespace ? ' ' : ''}
        if (text) {
          if (!inPre && whitespaceOption === 'condense') {text = text.replace(whitespaceRE$1, ' ')}
          var res
          var child
  ...
  function processRawAttrs (el) {
    ...
    if (len) {
      ...
      for (var i = 0; i < len; i++) {
        attrs[i] = { name: list[i].name, value: JSON.stringify(list[i].value) }
  ...
  function processFor (el) {
    ...
    if ((exp = getAndRemoveAttr(el, 'v-for'))) {
      ...
      if (res) {extend(el, res)} else {warn$2(('Invalid v-for expression: ' + exp), el.rawAttrsMap['v-for'])}
    }
  }
  ...
  function processIfConditions (el, parent) {
    ...
    if (prev && prev.if) {addIfCondition(prev, { exp: el.elseif, block: el })} else {
  ...
  function findPrevElement (children) {
    ...
    while (i--) {
      if (children[i].type === 1) {return children[i]} else {
        if (children[i].text !== ' ') {
  ...
  function processSlotContent (el) {
    ...
    }
    {
      if (el.tag === 'template') {
        ...
        if (slotBinding) {
          {
            if (el.slotTarget || el.slotScope) {warn$2('Unexpected mixed usage of different slot syntaxes.', el)}
        ...
        if (slotBinding$1) {
          {
            if (!maybeComponent(el)) {warn$2('v-slot can only be used on components or <template>.', slotBinding$1)}
  ...
  function getSlotName (binding) {
    ...
    if (!name) {
      if (binding.name[0] !== '#') {name = 'default'} else {
        warn$2('v-slot shorthand syntax requires a slot name.', binding)
      }
    }
    return dynamicArgRE.test(name) ? { name: name.slice(1, -1), dynamic: true } : {
      name: ('"' + name + '"'),
      dynamic: false,
    }
  }
        ...
        if (bindRE.test(name)) {
          ...
          if (modifiers) {
            ...
            if (modifiers.sync) {
              ...
              } else {addHandler(el, ('"update:"+(' + name + ')'), syncGen, null, false, warn$2, list[i], true)}
            }
          }
  ...
  function isStatic (node) {
    ...
    return !!(node.pre ||
      (!node.hasBindings && !node.if && !node.for && !isBuiltInTag(node.tag) && isPlatformReservedTag(node.tag) &&
  ...
  function genHandler (handler) {
    ...
    if (!handler.modifiers) {
      ...
      for (var key in handler.modifiers) {
        if (modifierCode[key]) {
          ...
          genModifierCode += genGuard(
            ['ctrl', 'shift', 'alt', 'meta'].filter(function (keyModifier) {return !modifiers[keyModifier]}).
      ...
      var handlerCode = isMethodPath ? ('return ' + (handler.value) + '($event)') : isFunctionExpression ? ('return (' +
        (handler.value) + ')($event)') : isFunctionInvocation ? ('return ' + (handler.value)) : handler.value
  ...
  function genElement (el, state) {
    ...
    if (el.staticRoot && !el.staticProcessed) {return genStatic(el, state)} else if (el.once &&
      !el.onceProcessed) {return genOnce(el, state)} else if (el.for && !el.forProcessed) {
      return genFor(el, state)
    } else if (el.if && !el.ifProcessed) {return genIf(el, state)} else if (el.tag === 'template' && !el.slotTarget &&
      !state.pre) {return genChildren(el, state) || 'void 0'} else if (el.tag === 'slot') {
  ...
  function genInlineTemplate (el, state) {
    ...
    if (ast && ast.type === 1) {
      ...
      return ('inlineTemplate:{render:function(){' + (inlineRenderFns.render) + '},staticRenderFns:[' +
        (inlineRenderFns.staticRenderFns.map(function (code) {return ('function(){' + code + '}')}).join(',')) + ']}')
    }
  }
  ...
  function genScopedSlots (el, slots, state) {
    ...
    var generatedSlots = Object.keys(slots).map(function (key) {return genScopedSlot(slots[key], state)}).join(',')
  ...
  function genScopedSlot (el, state) {
    ...
    var fn = 'function(' + slotScope + '){' +
      'return ' + (el.tag === 'template' ? el.if && isLegacySyntax ? ('(' + (el.if) + ')?' +
        (genChildren(el, state) || 'undefined') + ':undefined') : genChildren(el, state) || 'undefined' : genElement(el,
        state)) + '}'
  ...
  function getNormalizationType (children, maybeComponent) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (maybeComponent(el) ||
        (el.ifConditions && el.ifConditions.some(function (c) {return maybeComponent(c.block)}))) {res = 1}
    }
    return res
  }
  ...
  function genNode (node, state) {
    if (node.type === 1) {return genElement(node, state)} else if (node.type === 3 &&
      node.isComment) {return genComment(node)} else {return genText(node)}
  }
  ...
  function genSlot (el, state) {
    ...
    var attrs = el.attrs || el.dynamicAttrs ? genProps((el.attrs || []).concat(el.dynamicAttrs || []).
      map(function (attr) {return ({ name: camelize(attr.name), value: attr.value, dynamic: attr.dynamic })})) : null
  ...
  function genProps (props) {
    ...
    for (var i = 0; i < props.length; i++) {
      ...
      if (prop.dynamic) {dynamicProps += (prop.name) + ',' + value + ','} else {
  ...
  var prohibitedKeywordRE = new RegExp(
    '\\b' + ('do,if,for,let,new,try,var,case,else,with,await,break,catch,class,const,' +
      ...
      for (var name in node.attrsMap) {
        if (dirRE.test(name)) {
          ...
          if (value) {
            ...
            if (name === 'v-for') {checkFor(node, ('v-for="' + value + '"'), warn, range)} else if (onRE.test(
              name)) {checkEvent(value, (name + '="' + value + '"'), warn, range)} else {
  ...
  function checkIdentifier (ident, type, text, warn, range) {
    if (typeof ident === 'string') {
      ...
      } catch (e) {warn(('invalid ' + type + ' "' + ident + '" in expression: ' + (text.trim())), range)}
    }
  }
  
  function checkExpression (exp, text, warn, range) {
    try {new Function(('return ' + exp))} catch (e) {
  ...
  function createFunction (code, errors) {
    try {return new Function(code)} catch (err) {
