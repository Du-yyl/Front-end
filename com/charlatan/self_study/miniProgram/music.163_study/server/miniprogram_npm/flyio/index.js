								...
								value: function request (url, data, options) {
									...
									var promise = new Promise (function (resolve, reject) {
										...
										function makeRequest (options) {
											...
											var needQuery = ['GET', 'HEAD', 'DELETE', 'OPTION'].indexOf (
													options.method) !== -1
											...
											var customContentType = options.headers[contentType] ||
													options.headers[contentTypeLowerCase]
											...
											if ( utils.trim ((customContentType || '').toLowerCase ()) ===
													_contentType ) {
												data = utils.formatParams (data)
											} else if ( !utils.isFormData (data) &&
													['object', 'array'].indexOf (utils.type (data)) !== -1 ) {
											...
											engine.onload = function () {
												try {
													...
													if ( response && options.parseJson &&
															(engine.getResponseHeader (contentType) || '').indexOf (
																	'json') !== -1
										...
										enqueueIfLocked (requestInterceptor.p, function () {
											...
											if ( requestInterceptorHandler ) {
												ret = requestInterceptorHandler.call (requestInterceptor, options,
														Promise) || options
											}
											if ( !isPromise (ret) ) {
