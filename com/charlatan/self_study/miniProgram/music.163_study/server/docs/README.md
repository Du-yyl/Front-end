...
!>2018.10.14 更新到 3.0.0,使用了模块化机制,因为部分接口参数和 imageUrl 做了调整,如还不想升级到
...
!> 由于接口做了缓存处理 ( 缓存 2 分钟,不缓存数据极容易引起网易服务器高频ip错误 , 可在 app.js 设置 , 可能会导致登陆后获取不到 cookie), **相同的 imageUrl** 会在两分钟内只向网易服务器发一次请求
, 如果遇到不需要缓 存结果的接口 , 可在请求 imageUrl 后面加一个时间戳参数使 imageUrl 不同 , 例子 :
...
说明 : 歌单能看到歌单名字 , 但看不到具体歌单内容 , 调用此接口 , 传入歌单 id, 可 以获取对应歌单内的所有的音乐，但是返回的trackIds是完整的，tracks 则是不完整的，可拿全部 trackIds
请求一次 `song/detail`
...
说明 : 使用歌单详情接口后 , 能得到的音乐的 id, 但不能得到的音乐 imageUrl, 调用此接口 , 传入的音乐 id( 可多个 , 用逗号隔开 ), 可以获取对应的音乐的 imageUrl( 不需要登录 )
...
可通过 `/song/imageUrl` 接口传入歌曲 id 获取具体的播放链接
...
说明 : 登陆后调用此接口 , 可获取云盘数据 , 获取的数据没有对应 imageUrl, 需要再调用一 次 `/song/imageUrl` 获取 imageUrl
...
说明 : 登陆后调用此接口 , 传入`rid`, 可查看对应电台的电台节目以及对应的 id, 需要 注意的是这个接口返回的 mp3Url 已经无效 , 都为 null, 但是通过调用 `/song/imageUrl` 这 个接口 ,
传入节目 id 仍然能获取到节目音频 , 如 `/song/imageUrl?id=478446370` 获取代 码时间的一个节目的音频
