...
```javascript
// 例子
const imageUrl = `http://localhost:3000/song/imageUrl?id=33894312&proxy=http://121.196.226.246:84`
fetch(imageUrl).then(function() {
  // do what you want
})

// 结果
// {"data":[{"id":33894312,"imageUrl":"http://m10.music.126.net/20180104125640/930a968b3fb04908b733506b3833e60b/ymusic/0fd6/4f65/43ed/a8772889f38dfcb91c04da915b301617.mp3","br":320000,"size":10691439,"md5":"a8772889f38dfcb91c04da915b301617","code":200,"expi":1200,"type":"mp3","gain":-2.0E-4,"fee":0,"uf":null,"payed":0,"flag":0,"canExtend":false}],"code": 200}
```
v3.3.0 后支持使用 PAC代理,如 `?proxy=http://192.168.0.1/proxy.pac`

## 更新到 v3.0 说明

!>2018.10.14 更新到 3.0.0,使用了模块化机制,因为部分接口参数和 imageUrl 做了调整,如还不想升级到 3.0.0,请查看 [v2 的文档](http://binaryify.github.io/NeteaseCloudMusicApi/#/v2), [更新日志](https://github.com/Binaryify/NeteaseCloudMusicApi/blob/master/CHANGELOG.MD), [2.0+下载地址](https://github.com/Binaryify/NeteaseCloudMusicApi/releases/tag/v2.20.5), 同时 2.0+ 将不再维护
...
!> 由于接口做了缓存处理 ( 缓存 2 分钟,不缓存数据极容易引起网易服务器高频ip错误 , 可在 app.js 设置 , 可能会导致登陆后获取不到 cookie), **相同的 imageUrl** 会在两分钟内只向网易服务器发一次请求 , 如果遇到不需要缓
存结果的接口 , 可在请求 imageUrl 后面加一个时间戳参数使 imageUrl 不同 , 例子 :
...
说明 : 使用歌单详情接口后 , 能得到的音乐的 id, 但不能得到的音乐 imageUrl, 调用此接口
, 传入的音乐 id( 可多个 , 用逗号隔开 ), 可以获取对应的音乐的 imageUrl( 不需要登录 )

> 注 : 部分用户反馈获取的 imageUrl 会 403,[hwaphon](https://github.com/hwaphon)找到的
> 解决方案是当获取到音乐的 id 后，将
> https://music.163.com/song/media/outer/imageUrl?id=id.mp3 以 src 赋予 Audio 即可播放
...
**调用例子 :** `/song/imageUrl?id=33894312` `/song/imageUrl?id=405998841,33894312`

返回数据如下图 :
![音乐 imageUrl](https://raw.githubusercontent.com/Binaryify/NeteaseCloudMusicApi/master/static/%E9%9F%B3%E4%B9%90%20url.png)
...
mp3url 不能直接用 , 可通过 `/song/imageUrl` 接口传入歌曲 id 获取具体的播放链接
...
**调用例子 :** `/video/imageUrl?id=89ADDE33C0AAE8EC14B99F6750DB954D`
...
说明 : 登陆后调用此接口 , 可获取云盘数据 , 获取的数据没有对应 imageUrl, 需要再调用一
次 `/song/imageUrl` 获取 imageUrl
...
注意的是这个接口返回的 mp3Url 已经无效 , 都为 null, 但是通过调用 `/song/imageUrl` 这
个接口 , 传入节目 id 仍然能获取到节目音频 , 如 `/song/imageUrl?id=478446370` 获取代
码时间的一个节目的音频
