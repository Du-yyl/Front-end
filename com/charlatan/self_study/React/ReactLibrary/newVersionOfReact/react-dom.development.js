...
(function (global, factory) {
	...
	function sanitizeURL (url) {
		{
			if ( !didWarn && isJavaScriptProtocol.test (url) ) {
				...
				error ('A future version of React will block javascript: URLs as a security precaution. ' +
						'Use event handlers instead if you can. If you need to generate unsafe HTML try ' +
						'using dangerouslySetInnerHTML instead. React was passed %s.', JSON.stringify (url))
			}
		}
	}
	...
	function checkControlledValueProps (tagName, props) {
		{
			if ( !(hasReadOnlyValue[props.type] || props.onChange || props.onInput || props.readOnly ||
					props.disabled || props.value == null) ) {
				error ('You provided a `value` prop to a form field without an ' +
			...
			if ( !(props.onChange || props.readOnly || props.disabled || props.checked == null) ) {
				error ('You provided a `checked` prop to a form field without an ' +
						'`onChange` handler. This will render a read-only field. If ' +
	...
	function trackValueOnNode (node) {
		...
		if ( node.hasOwnProperty (valueField) || typeof descriptor === 'undefined' || typeof descriptor.get !==
				'function' || typeof descriptor.set !== 'function' ) {
			return
		}
	...
	function initWrapperState (element, props) {
		{
			...
			if ( props.checked !== undefined && props.defaultChecked !== undefined && !didWarnCheckedDefaultChecked ) {
				error ('%s contains an input of type %s with both checked and defaultChecked props. ' +
	...
	function checkSelectPropTypes (props) {
		{
			...
			for ( var i = 0; i < valuePropNames.length; i++ ) {
				...
				if ( props.multiple && !isArray ) {
					error ('The `%s` prop supplied to <select> must be an array if ' + '`multiple` is true.%s',
							propName, getDeclarationErrorAddendum ())
				} else if ( !props.multiple && isArray ) {
					error ('The `%s` prop supplied to <select> must be a scalar ' + 'value if `multiple` is false.%s',
							propName, getDeclarationErrorAddendum ())
				}
			}
		}
	...
	function initWrapperState$1 (element, props) {
		...
		
		{
			if ( props.value !== undefined && props.defaultValue !== undefined && !didWarnValueDefaultValue$1 ) {
				error ('Select elements must be either controlled or uncontrolled ' +
						'(specify either the value prop, or the defaultValue prop, but not ' +
	...
	function validateShorthandPropertyCollisionInDev (styleUpdates, nextStyles) {
		{
			...
			for ( var key in expandedUpdates ) {
				...
				if ( correctOriginalKey && originalKey !== correctOriginalKey ) {
					...
					error ('%s a style property during rerender (%s) when a ' +
	...
	function validateProperty (tagName, name) {
		{
			...
			if ( rARIACamel.test (name) ) {
				...
				if ( correctName == null ) {
					error ('Invalid ARIA attribute `%s`. ARIA attributes follow the pattern aria-* and must be lowercase.',
							name)
	...
	function warnInvalidARIAProps (type, props) {
		{
			...
			if ( invalidProps.length === 1 ) {
				error ('Invalid aria prop %s on <%s> tag. ' +
						'For details, see https://reactjs.org/link/invalid-aria-props', unknownPropString, type)
			} else if ( invalidProps.length > 1 ) {
				error ('Invalid aria props %s on <%s> tag. ' +
						'For details, see https://reactjs.org/link/invalid-aria-props', unknownPropString, type)
			}
		}
	}
	...
	function validateProperties$1 (type, props) {
		{
			...
			if ( props != null && props.value === null && !didWarnValueNull ) {
				...
				if ( type === 'select' && props.multiple ) {
					error ('`value` prop on `%s` should not be null. ' +
		...
		validateProperty$1 = function (tagName, name, value, eventRegistry) {
			...
			if ( eventRegistry != null ) {
				...
				if ( INVALID_EVENT_NAME_REGEX.test (name) ) {
					error ('Invalid event handler property `%s`. ' +
			...
			if ( lowerCasedName === 'innerhtml' ) {
				error ('Directly setting property `innerHTML` is not permitted. ' +
			...
			if ( lowerCasedName === 'aria' ) {
				error ('The `aria` attribute is reserved for future use in React. ' +
			...
			if ( typeof value === 'number' && isNaN (value) ) {
				error ('Received NaN for the `%s` attribute. If this is expected, cast ' + 'the value to a string.',
						name)
			...
			if ( (value === 'false' || value === 'true') && propertyInfo !== null && propertyInfo.type === BOOLEAN ) {
				error ('Received the string `%s` for the boolean attribute `%s`. ' + '%s ' + 'Did you mean %s={%s}?',
	...
	var warnUnknownProperties = function (type, props, eventRegistry) {
		{
			...
			if ( unknownProps.length === 1 ) {
				error ('Invalid value for prop %s on <%s> tag. Either remove it from the element, ' +
						...
						'For details, see https://reactjs.org/link/attribute-behavior ', unknownPropString, type)
			} else if ( unknownProps.length > 1 ) {
				error ('Invalid values for props %s on <%s> tag. Either remove them from the element, ' +
	...
	var SHOULD_NOT_PROCESS_POLYFILL_EVENT_PLUGINS = IS_EVENT_HANDLE_NON_MANAGED_NODE | IS_NON_DELEGATED |
			IS_CAPTURE_PHASE
	...
	function getListener (inst, registrationName) {
		...
		if ( !( !listener || typeof listener === 'function') ) {
			{
				throw Error ('Expected `' + registrationName + '` listener to be a function, instead got a value of `' +
	...
	function isMounted (component) {
		{
			...
			if ( owner !== null && owner.tag === ClassComponent ) {
				...
				if ( !instance._warnedAboutRefsInRender ) {
					error ('%s is accessing isMounted inside its render() function. ' +
	...
	function queueDiscreteEvent (blockedOn, domEventName, eventSystemFlags, targetContainer, nativeEvent) {
		var queuedEvent = createQueuedReplayableEvent (blockedOn, domEventName, eventSystemFlags, targetContainer,
				nativeEvent)
	...
	function queueIfContinuousEvent (blockedOn, domEventName, eventSystemFlags, targetContainer, nativeEvent) {
		...
		switch (domEventName) {
			case 'focusin': {
				...
				queuedFocus = accumulateOrCreateContinuousQueuedReplayableEvent (queuedFocus, blockedOn, domEventName,
						eventSystemFlags, targetContainer, focusEvent)
				return true
			}
			
			case 'dragenter': {
				var dragEvent = nativeEvent
				queuedDrag = accumulateOrCreateContinuousQueuedReplayableEvent (queuedDrag, blockedOn, domEventName,
						eventSystemFlags, targetContainer, dragEvent)
				return true
			}
			
			case 'mouseover': {
				var mouseEvent = nativeEvent
				queuedMouse = accumulateOrCreateContinuousQueuedReplayableEvent (queuedMouse, blockedOn, domEventName,
						eventSystemFlags, targetContainer, mouseEvent)
				return true
			}
			...
			case 'gotpointercapture': {
				...
				queuedPointerCaptures.set (_pointerId2, accumulateOrCreateContinuousQueuedReplayableEvent (
						queuedPointerCaptures.get (_pointerId2) || null, blockedOn, domEventName, eventSystemFlags,
						targetContainer, _pointerEvent))
				return true
			}
		}
	...
	function attemptReplayContinuousQueuedEvent (queuedEvent) {
		...
		while ( targetContainers.length > 0 ) {
			...
			var nextBlockedOn = attemptToDispatchEvent (queuedEvent.domEventName, queuedEvent.eventSystemFlags,
	...
	function replayUnblockedEvents () {
		...
		while ( queuedDiscreteEvents.length > 0 ) {
			...
			while ( targetContainers.length > 0 ) {
				var targetContainer = targetContainers[0]
				var nextBlockedOn = attemptToDispatchEvent (nextDiscreteEvent.domEventName,
						nextDiscreteEvent.eventSystemFlags, targetContainer, nextDiscreteEvent.nativeEvent)
	...
	function dispatchUserBlockingUpdate (domEventName, eventSystemFlags, container, nativeEvent) {
		{
			runWithPriority (UserBlockingPriority$1,
	...
	function extractEvents$2 (
		...
		var leave = new SyntheticEventCtor (leaveEventType, eventTypePrefix + 'leave', from, nativeEvent,
				nativeEventTarget)
		...
		if ( nativeTargetInst === targetInst ) {
			var enterEvent = new SyntheticEventCtor (enterEventType, eventTypePrefix + 'enter', to, nativeEvent,
					nativeEventTarget)
	...
	function extractEvents$4 (
		...
		
		{
			...
			var _listeners = accumulateSinglePhaseListeners (targetInst, reactName, nativeEvent.type, inCapturePhase,
	...
	function addTrappedEventListener (
		...
		if ( isCapturePhaseListener ) {
			if ( isPassiveListener !== undefined ) {
				unsubscribeListener = addEventCaptureListenerWithPassiveFlag (targetContainer, domEventName, listener,
						isPassiveListener)
			} else {
			...
			if ( isPassiveListener !== undefined ) {
				unsubscribeListener = addEventBubbleListenerWithPassiveFlag (targetContainer, domEventName, listener,
						isPassiveListener)
			} else {
	...
	function dispatchEventForPluginEventSystem (
		...
		if ( (eventSystemFlags & IS_EVENT_HANDLE_NON_MANAGED_NODE) === 0 && (eventSystemFlags & IS_NON_DELEGATED) ===
				0 ) {
	...
	
	{
		...
		warnForPropDifference = function (propName, serverValue, clientValue) {
			...
			error ('Prop `%s` did not match. Server: %s Client: %s', propName, JSON.stringify (normalizedServerValue),
		...
		warnForInvalidEventListener = function (registrationName, listener) {
			if ( listener === false ) {
				...
				error ('Expected `%s` listener to be a function, instead got a value of `%s` type.', registrationName,
						typeof listener)
			}
		...
		normalizeHTML = function (parent, html) {
			...
			var testElement = parent.namespaceURI === HTML_NAMESPACE$1 ? parent.ownerDocument.createElement (
					parent.tagName) : parent.ownerDocument.createElementNS (parent.namespaceURI, parent.tagName)
		...
		for ( var propKey in nextProps ) {
			...
			if ( propKey === STYLE ) {
				...
				if ( typeof nextProp === 'string' ) {
					...
					AUTOFOCUS )  else if ( registrationNameDependencies.hasOwnProperty (propKey) ) {
				if ( nextProp != null ) {
		...
		if ( namespaceURI === HTML_NAMESPACE$1 ) {
			{
				...
				if ( !isCustomComponentTag && type !== type.toLowerCase () ) {
					error ('<%s /> is using incorrect casing. ' + 'Use PascalCase for React components, ' +
	...
	function diffProperties (domElement, tag, lastRawProps, nextRawProps, rootContainerElement) {
		...
		for ( propKey in lastProps ) {
			...
			} else if ( propKey === DANGEROUSLY_SET_INNER_HTML || propKey === CHILDREN )  else if ( propKey ===
					SUPPRESS_CONTENT_EDITABLE_WARNING || propKey ===
					SUPPRESS_HYDRATION_WARNING )  else if ( propKey ===
					AUTOFOCUS )  else if ( registrationNameDependencies.hasOwnProperty (propKey) ) {
		...
		for ( propKey in nextProps ) {
			...
			if ( propKey === STYLE ) {
				...
				if ( typeof nextProp === 'string' || typeof nextProp === 'number' ) {
					...
					SUPPRESS_HYDRATION_WARNING )  else if ( registrationNameDependencies.hasOwnProperty (propKey) ) {
				if ( nextProp != null ) {
			...
			} else if ( typeof nextProp === 'object' && nextProp !== null && nextProp.$$typeof ===
	...
	function updateProperties (domElement, updatePayload, tag, lastRawProps, nextRawProps) {
		...
		var isCustomComponentTag = isCustomComponent (tag, nextRawProps) // Apply the 8、diff.
	...
	function diffHydratedProperties (domElement, tag, rawProps, parentNamespace, rootContainerElement) {
		...
		for ( var propKey in rawProps ) {
			...
			if ( propKey === CHILDREN ) {
				...
				if ( suppressHydrationWarning )  else if ( propKey === SUPPRESS_CONTENT_EDITABLE_WARNING || propKey ===
						SUPPRESS_HYDRATION_WARNING || // Controlled attributes are not validated
						// TODO: Only ignore them on controlled tags.
						propKey === 'value' || propKey === 'checked' || propKey === 'selected' )  else if ( propKey ===
	...
	function warnForDeletedHydratableElement (parentNode, child) {
		{
			...
			error ('Did not expect server HTML to contain a <%s> in <%s>.', child.nodeName.toLowerCase (),
	...
	function warnForDeletedHydratableText (parentNode, child) {
		{
			...
			error ('Did not expect server HTML to contain the text node "%s" in <%s>.', child.nodeValue,
					parentNode.nodeName.toLowerCase ())
		}
	}
	...
	function warnForInsertedHydratedText (parentNode, text) {
		{
			...
			error ('Expected server HTML to contain a matching text node for "%s" in <%s>.', text,
					parentNode.nodeName.toLowerCase ())
		}
	}
			...
			switch (parentTag) {
				...
				case 'head':
					return tag === 'base' || tag === 'basefont' || tag === 'bgsound' || tag === 'link' || tag ===
							'meta' || tag === 'title' || tag === 'noscript' || tag === 'noframes' || tag === 'style' ||
		...
		validateDOMNesting = function (childTag, childText, ancestorInfo) {
			...
			if ( invalidParent ) {
				...
				if ( ancestorTag === 'table' && childTag === 'tr' ) {
					info += ' Add a <tbody>, <thead> or <tfoot> to your code to match the DOM tree generated by ' +
							'the browser.'
				}
				
				error ('validateDOMNesting(...): %s cannot appear as a child of <%s>.%s%s', tagDisplayName, ancestorTag,
						whitespaceInfo, info)
			} else {
				error ('validateDOMNesting(...): %s cannot appear as a descendant of ' + '<%s>.', tagDisplayName,
						ancestorTag)
			}
		}
	}
	...
	function commitUpdate (domElement, updatePayload, type, oldProps, newProps, internalInstanceHandle) {
		...
		updateFiberProps (domElement, newProps) // Apply the 8、diff to the DOM node.
	...
	function getNextHydratableInstanceAfterSuspenseInstance (suspenseInstance) {
		...
		while ( node ) {
			if ( node.nodeType === COMMENT_NODE ) {
				...
				} else if ( data === SUSPENSE_START_DATA || data === SUSPENSE_FALLBACK_START_DATA || data ===
	...
	function getInstanceFromNode (node) {
		...
		if ( inst ) {
			if ( inst.tag === HostComponent || inst.tag === HostText || inst.tag === SuspenseComponent || inst.tag ===
					HostRoot ) {
				return inst
			} else {
				return null
	...
	function processChildContext (fiber, type, parentContext) {
		{
			...
			if ( typeof instance.getChildContext !== 'function' ) {
				{
					...
					if ( !warnedAboutMissingGetChildContext[componentName] ) {
						...
						error ('%s.childContextTypes is specified but there is no getChildContext() method ' +
			...
			for ( var contextKey in childContext ) {
				if ( !(contextKey in childContextTypes) ) {
					{
						throw Error (
								(getComponentName (type) || 'Unknown') + '.getChildContext(): key "' + contextKey +
	...
	function scheduleSyncCallback (callback) {
		...
		if ( syncQueue === null ) {
			...
			immediateQueueCallbackNode = Scheduler_scheduleCallback (Scheduler_ImmediatePriority,
	...
	function calculateChangedBits (context, newValue, oldValue) {
		if ( objectIs (oldValue, newValue) ) {
			...
			var changedBits = typeof context._calculateChangedBits === 'function' ? context._calculateChangedBits (
					oldValue, newValue) : MAX_SIGNED_31_BIT_INT
			
			{
				if ( (changedBits & MAX_SIGNED_31_BIT_INT) !== changedBits ) {
					error ('calculateChangedBits: Expected the return value to be a ' +
							'31-bit integer. Instead received: %s', changedBits)
				}
			}
	...
	function readContext (context, observedBits) {
		...
		if ( lastContextWithAllBitsObserved === context )  else if ( observedBits === false || observedBits ===
				0 )  else {
	...
	function checkClassInstance (workInProgress, ctor, newProps) {
		...
		
		{
			...
			if ( instance.getInitialState && !instance.getInitialState.isReactClassApproved && !instance.state ) {
				error ('getInitialState was defined on %s, a plain JavaScript class. ' +
						'This is only supported for classes created using React.createClass. ' +
						'Did you mean to define a state property instead?', name)
			}
			
			if ( instance.getDefaultProps && !instance.getDefaultProps.isReactClassApproved ) {
				error ('getDefaultProps was defined on %s, a plain JavaScript class. ' +
						'This is only supported for classes created using React.createClass. ' +
			...
			if ( typeof instance.componentShouldUpdate === 'function' ) {
				error ('%s has a method called ' + 'componentShouldUpdate(). Did you mean shouldComponentUpdate()? ' +
			...
			if ( typeof instance.componentDidUnmount === 'function' ) {
				error ('%s has a method called ' + 'componentDidUnmount(). But there is no such lifecycle method. ' +
			...
			if ( instance.props !== undefined && hasMutatedProps ) {
				error ('%s(...): When calling super() in `%s`, make sure to pass ' +
						'up the same props that your component\'s constructor was passed.', name, name)
			}
			
			if ( instance.defaultProps ) {
				error ('Setting defaultProps as an instance property on %s is not supported and will be ignored.' +
			...
			if ( typeof instance.getSnapshotBeforeUpdate === 'function' && typeof instance.componentDidUpdate !==
					'function' && !didWarnAboutGetSnapshotBeforeUpdateWithoutDidUpdate.has (ctor) ) {
		...
		
		{
			if ( 'contextType' in ctor ) {
				...
				if ( !isValid && !didWarnAboutInvalidateContextType.has (ctor) ) {
					...
					error ('%s defines an invalid contextType. ' +
							'contextType should point to the Context object returned by React.createContext().%s',
							getComponentName (ctor) || 'Component', addendum)
				}
			}
		}
		...
		
		{
			if ( typeof ctor.getDerivedStateFromProps === 'function' && state === null ) {
				...
				if ( !didWarnAboutUninitializedState.has (componentName) ) {
					...
					error ('`%s` uses `getDerivedStateFromProps` but its initial state is ' +
				...
				if ( foundWillMountName !== null || foundWillReceivePropsName !== null || foundWillUpdateName !==
					...
					if ( !didWarnAboutLegacyLifecyclesAndDerivedState.has (_componentName) ) {
						...
						error ('Unsafe legacy lifecycles will not be called for components using new component APIs.\n\n' +
								...
								'https://reactjs.org/link/unsafe-component-lifecycles', _componentName, newApiName,
		...
		if ( instance.state !== oldState ) {
			{
				...
				if ( !didWarnAboutStateAssignmentForComponent.has (componentName) ) {
					...
					error ('%s.componentWillReceiveProps(): Assigning directly to ' +
		...
		
		{
			if ( instance.state === newProps ) {
				...
				if ( !didWarnAboutDirectlyAssigningPropsToState.has (componentName) ) {
					...
					error ('%s: It is not recommended to assign props directly to state ' +
	...
	function updateClassInstance (current, workInProgress, ctor, newProps, renderLanes) {
		...
		var oldProps = workInProgress.type === workInProgress.elementType ? unresolvedOldProps : resolveDefaultProps (
				workInProgress.type, unresolvedOldProps)
		instance.props = oldProps
	...
	function coerceRef (returnFiber, current, element) {
		...
		if ( mixedRef !== null && typeof mixedRef !== 'function' && typeof mixedRef !== 'object' ) {
			...
			if ( element._owner ) {
				...
				if ( !inst ) {
					{
						throw Error ('Missing owner for string ref ' + mixedRef +
								'. This error is likely caused by a bug in React. Please file an issue.')
					}
				}
				...
				if ( !element._owner ) {
					{
						throw Error ('Element ref was specified as a string (' + mixedRef +
			...
			{
				{
					throw Error ('Objects are not valid as a React child (found: ' +
							(Object.prototype.toString.call (newChild) === '[object Object]' ? 'object with keys {' +
									Object.keys (newChild).join (', ') + '}' : newChild) +
	...
	function ChildReconciler (shouldTrackSideEffects) {
		...
		function updatePortal (returnFiber, current, portal, lanes) {
			if ( current === null || current.tag !== HostPortal || current.stateNode.containerInfo !==
					portal.containerInfo || current.stateNode.implementation !== portal.implementation ) {
				// Insert
		...
		function updateFromMap (existingChildren, returnFiber, newIdx, newChild, lanes) {
			...
			if ( typeof newChild === 'object' && newChild !== null ) {
				switch (newChild.$$typeof) {
					case REACT_ELEMENT_TYPE: {
						var _matchedFiber = existingChildren.get (newChild.key === null ? newIdx : newChild.key) ||
								null
						
						if ( newChild.type === REACT_FRAGMENT_TYPE ) {
							return updateFragment (returnFiber, _matchedFiber, newChild.props.children, lanes,
									newChild.key)
						}
		...
		function reconcileChildrenIterator (returnFiber, currentFirstChild, newChildrenIterable, lanes) {
			...
			
			{
				...
				if ( newChildrenIterable.entries === iteratorFn ) {
					if ( !didWarnAboutMaps ) {
						error ('Using Maps as children is not supported. ' +
	...
	function prepareToHydrateHostInstance (fiber, rootContainerInstance, hostContext) {
		...
		var updatePayload = hydrateInstance (instance, fiber.type, fiber.memoizedProps, rootContainerInstance,
				hostContext, fiber) // TODO: Type this specific to this type of component.
		...
		
		{
			...
			if ( nextDeps.length !== prevDeps.length ) {
				error ('The final argument passed to %s changed size between renders. The ' +
						...
						currentHookNameInDev, '[' + prevDeps.join (', ') + ']', '[' + nextDeps.join (', ') + ']')
			}
		}
		...
		if ( pendingQueue !== null ) {
			...
			
			{
				if ( current.baseQueue !== baseQueue ) {
					...
					error ('Internal error: Expected work-in-progress queue to be a clone. ' +
		...
		if ( isSafeToReadFromSource ) {
			...
			
			{
				if ( typeof snapshot === 'function' ) {
					error ('Mutable source should not return a function as the snapshot value. ' +
	...
	function useMutableSource (hook, source, getSnapshot, subscribe) {
		...
		if ( !objectIs (prevGetSnapshot, getSnapshot) || !objectIs (prevSource, source) ||
	...
	function warnOnOpaqueIdentifierAccessInDEV (fiber) {
		{
			...
			if ( getIsRendering () && !didWarnAboutUseOpaqueIdentifier[name] ) {
				error ('The object passed back from useOpaqueIdentifier is meant to be ' +
	...
	function mountOpaqueIdentifier () {
		var makeId = makeClientIdInDEV.bind (null,
	...
	function updateMemoComponent (current, workInProgress, Component, nextProps, updateLanes, renderLanes) {
		if ( current === null ) {
			...
			if ( isSimpleFunctionComponent (type) && Component.compare === null && // SimpleMemoComponent codepath doesn't resolve outer props either.
				...
				return updateSimpleMemoComponent (current, workInProgress, resolvedType, nextProps, updateLanes,
						renderLanes)
			}
			...
			var child = createFiberFromTypeAndProps (Component.type, null, nextProps, workInProgress,
					workInProgress.mode, renderLanes)
	...
	function updateFunctionComponent (current, workInProgress, Component, nextProps, renderLanes) {
		...
		
		{
			...
			if ( workInProgress.mode & StrictMode ) {
				...
				try {
					nextChildren = renderWithHooks (current, workInProgress, Component, nextProps, context,
							renderLanes)
				} finally {
					reenableLogs ()
				}
	...
	function updateClassComponent (current, workInProgress, Component, nextProps, renderLanes) {
		...
		var nextUnitOfWork = finishClassComponent (current, workInProgress, Component, shouldUpdate, hasContext,
				renderLanes)
	...
	function mountLazyComponent (_current, workInProgress, elementType, updateLanes, renderLanes) {
		...
		{
			{
				throw Error ('Element type is invalid. Received a promise that resolves to: ' + Component +
	...
	function mountIndeterminateComponent (_current, workInProgress, Component, renderLanes) {
		...
		
		{
			...
			if ( typeof value === 'object' && value !== null && typeof value.render === 'function' && value.$$typeof ===
				...
				if ( !didWarnAboutModulePatternComponent[_componentName] ) {
					error ('The <%s /> component appears to be a function component that returns a class instance. ' +
		...
		if ( // Run these checks in production only if the flag is off.
				...
				typeof value === 'object' && value !== null && typeof value.render === 'function' && value.$$typeof ===
				undefined ) {
			{
				...
				if ( !didWarnAboutModulePatternComponent[_componentName2] ) {
					error ('The <%s /> component appears to be a function component that returns a class instance. ' +
							...
							'If you can\'t use a class try assigning the prototype on the function as a workaround. ' +
							'`%s.prototype = React.Component.prototype`. Don\'t use an arrow function since it ' +
							'cannot be called with `new` by React.', _componentName2, _componentName2, _componentName2)
	...
	function updateSuspenseComponent (current, workInProgress, renderLanes) {
		...
		if ( current === null ) {
			...
			if ( showFallback ) {
				var fallbackFragment = mountSuspenseFallbackChildren (workInProgress, nextPrimaryChildren,
						nextFallbackChildren, renderLanes)
				...
				// initial commit.
				var _fallbackFragment = mountSuspenseFallbackChildren (workInProgress, nextPrimaryChildren,
						nextFallbackChildren, renderLanes)
			...
			if ( prevState !== null ) {
				...
				if ( showFallback ) {
					...
					var _fallbackChildFragment = updateSuspenseFallbackChildren (current, workInProgress,
							_nextPrimaryChildren2, _nextFallbackChildren2, renderLanes)
					
					var _primaryChildFragment3 = workInProgress.child
					var prevOffscreenState = current.child.memoizedState
					_primaryChildFragment3.memoizedState = prevOffscreenState === null ? mountSuspenseOffscreenState (
							renderLanes) : updateSuspenseOffscreenState (prevOffscreenState, renderLanes)
					...
					var _primaryChildFragment4 = updateSuspensePrimaryChildren (current, workInProgress,
							_nextPrimaryChildren3, renderLanes)
				...
				if ( showFallback ) {
					...
					var _fallbackChildFragment2 = updateSuspenseFallbackChildren (current, workInProgress,
							_nextPrimaryChildren4, _nextFallbackChildren3, renderLanes)
					
					var _primaryChildFragment5 = workInProgress.child
					var _prevOffscreenState = current.child.memoizedState
					_primaryChildFragment5.memoizedState = _prevOffscreenState === null ? mountSuspenseOffscreenState (
							renderLanes) : updateSuspenseOffscreenState (_prevOffscreenState, renderLanes)
					...
					var _primaryChildFragment6 = updateSuspensePrimaryChildren (current, workInProgress,
							_nextPrimaryChildren5, renderLanes)
	...
	function validateRevealOrder (revealOrder) {
		{
			if ( revealOrder !== undefined && revealOrder !== 'forwards' && revealOrder !== 'backwards' &&
					revealOrder !== 'together' && !didWarnAboutRevealOrder[revealOrder] ) {
				...
				if ( typeof revealOrder === 'string' ) {
					switch (revealOrder.toLowerCase ()) {
						...
						default:
							error ('"%s" is not a supported revealOrder on <SuspenseList />. ' +
	...
	function validateTailOptions (tailMode, revealOrder) {
		{
			if ( tailMode !== undefined && !didWarnAboutTailOptions[tailMode] ) {
				if ( tailMode !== 'collapsed' && tailMode !== 'hidden' ) {
					...
					error ('"%s" is not a supported value for tail on <SuspenseList />. ' +
							'Did you mean "collapsed" or "hidden"?', tailMode)
	...
	function validateSuspenseListNestedChild (childSlot, index) {
		{
			...
			if ( isArray || isIterable ) {
				...
				error ('A nested %s was passed to row #%s in <SuspenseList />. Wrap it in ' +
						'an additional SuspenseList to configure its revealOrder: ' +
						'<SuspenseList revealOrder=...> ... ' +
						'<SuspenseList revealOrder=...>{%s}</SuspenseList> ... ' + '</SuspenseList>', type, index,
						type)
	...
	function validateSuspenseListChildren (children, revealOrder) {
		{
			if ( (revealOrder === 'forwards' || revealOrder === 'backwards') && children !== undefined && children !==
					null && children !== false ) {
				if ( Array.isArray (children) ) {
					...
					if ( typeof iteratorFn === 'function' ) {
						...
						error ('A single row was passed to a <SuspenseList revealOrder="%s" />. ' +
								...
								'Did you mean to pass multiple children or an array?', revealOrder)
					}
				}
			}
		}
	...
	function beginWork (current, workInProgress, renderLanes) {
						...
						if ( state !== null ) {
							...
							if ( includesSomeLane (renderLanes, primaryChildLanes) ) {
								...
								pushSuspenseContext (workInProgress,
										setDefaultShallowSuspenseContext (suspenseStackCursor.current)) // The primary children do not have pending work with sufficient
								// priority. Bailout.
		...
		switch (workInProgress.tag) {
			...
			case SimpleMemoComponent: {
				return updateSimpleMemoComponent (current, workInProgress, workInProgress.type,
						workInProgress.pendingProps, updateLanes, renderLanes)
			}
			...
			case IncompleteClassComponent: {
				...
				return mountIncompleteClassComponent (current, workInProgress, _Component3, _resolvedProps4,
						renderLanes)
			}
		...
		{
			{
				throw Error ('Unknown unit of work tag (' + workInProgress.tag +
						'). This error is likely caused by a bug in React. Please file an issue.')
			}
		}
	}
	...
	
	{
		...
		updateHostComponent$1 = function (current, workInProgress, type, newProps, rootContainerInstance) {
			...
			var updatePayload = prepareUpdate (instance, type, oldProps, newProps, rootContainerInstance,
					currentHostContext) // TODO: Type this specific to this type of component.
				...
				if ( current && workInProgress.stateNode != null ) {
					...
					if ( _wasHydrated2 ) {
						...
						workInProgress.stateNode = createTextInstance (newText, _rootContainerInstance,
								_currentHostContext, workInProgress)
					}
				}
						...
						if ( !cannotBeSuspended ) {
							...
							while ( row !== null ) {
								...
								if ( suspended !== null ) {
									...
									pushSuspenseContext (workInProgress,
											setShallowSuspenseContext (suspenseStackCursor.current,
					...
					if ( !didSuspendAlready ) {
						...
						if ( _suspended !== null ) {
							...
							if ( renderState.tail === null && renderState.tailMode === 'hidden' &&
								...
								// exceed it.
								now () * 2 - renderState.renderingStartTime > getRenderTargetTime () && renderLanes !==
								OffscreenLane ) {
	...
	function logCapturedError (boundary, errorInfo) {
		try {
			...
			if ( true ) {
				...
				var componentNameMessage = componentName ? 'The above error occurred in the <' + componentName +
	...
	function throwException (root, returnFiber, sourceFiber, value, rootRenderLanes) {
		...
		if ( value !== null && typeof value === 'object' && typeof value.then === 'function' ) {
			...
			var hasInvisibleParentBoundary = hasSuspenseContext (suspenseStackCursor.current,
					InvisibleParentSuspenseContext) // Schedule the nearest Suspense to re-render the timed out view.
					...
					if ( current !== null ) {
						...
						var snapshot = instance.getSnapshotBeforeUpdate (
								finishedWork.elementType === finishedWork.type ? prevProps : resolveDefaultProps (
										finishedWork.type, prevProps), prevState)
					...
					
					{
						...
						if ( destroy !== undefined && typeof destroy !== 'function' ) {
							...
							error ('An effect function must not return anything besides a function, ' +
									'which is used for clean-up.%s', addendum)
						}
					}
				}
			...
			case Profiler: {
				{
					...
					if ( typeof onRender === 'function' ) {
						{
							onRender (finishedWork.memoizedProps.id, current === null ? 'mount' : 'update',
									finishedWork.actualDuration, finishedWork.treeBaseDuration,
									finishedWork.actualStartTime, commitTime, finishedRoot.memoizedInteractions)
						}
					}
				}
	...
	function hideOrUnhideAllChildren (finishedWork, isHidden) {
		{
			...
			while ( true ) {
				...
				} else if ( (node.tag === OffscreenComponent || node.tag === LegacyHiddenComponent) &&
						node.memoizedState !== null && node !== finishedWork )  else if ( node.child !== null ) {
					node.child.return = node
					node = node.child
					continue
				}
	...
	function requestUpdateLane (fiber) {
		...
		if ( // TODO: Temporary. We're removing the concept of discrete updates.
				(executionContext & DiscreteEventContext) !== NoContext && schedulerPriority ===
	...
	function renderDidSuspendDelayIfPossible () {
		...
		if ( workInProgressRoot !== null && (includesNonIdleWork (workInProgressRootSkippedLanes) ||
	...
	function resetChildLanes (completedWork) {
		if ( // TODO: Move this check out of the hot path by moving `resetChildLanes`
				...
				completedWork.memoizedState !== null && !includesSomeLane (subtreeRenderLanes, OffscreenLane) &&
		...
		if ( (completedWork.mode & ProfileMode) !== NoMode ) {
			...
			var shouldBubbleActualDurations = completedWork.alternate === null || completedWork.child !==
		...
		while ( nextEffect !== null ) {
			...
			if ( !shouldFireAfterActiveInstanceBlur && focusedInstanceHandle !== null ) {
				if ( (nextEffect.flags & Deletion) !== NoFlags ) {
					...
					if ( nextEffect.tag === SuspenseComponent && isSuspenseBoundaryBeingHidden (current, nextEffect) &&
	...
	function pingSuspendedRoot (root, wakeable, pingedLanes) {
		...
		if ( workInProgressRoot === root && isSubsetOfLanes (workInProgressRootRenderLanes, pingedLanes) ) {
			...
			if ( workInProgressRootExitStatus === RootSuspendedWithDelay || workInProgressRootExitStatus ===
					RootSuspended && includesOnlyRetries (workInProgressRootRenderLanes) && now () -
	...
	function jnd (timeElapsed) {
		return timeElapsed < 120 ? 120 : timeElapsed < 480 ? 480 : timeElapsed < 1080 ? 1080 : timeElapsed < 1920
				? 1920
	...
	function warnAboutUpdateOnNotYetMountedFiberInDEV (fiber) {
		{
			...
			if ( tag !== IndeterminateComponent && tag !== HostRoot && tag !== ClassComponent && tag !==
					FunctionComponent && tag !== ForwardRef && tag !== MemoComponent && tag !== SimpleMemoComponent &&
					tag !== Block ) {
	...
	function warnAboutUpdateOnUnmountedFiberInDEV (fiber) {
		{
			...
			if ( tag !== HostRoot && tag !== ClassComponent && tag !== FunctionComponent && tag !== ForwardRef &&
			...
			if ( isFlushingPassiveEffects )  else {
				...
				try {
					...
					error ('Can\'t perform a React state update on an unmounted component. This ' +
							'is a no-op, but it indicates a memory leak in your application. To ' +
							'fix, cancel all subscriptions and asynchronous tasks in %s.', tag === ClassComponent
	...
	
	{
		...
		beginWork$1 = function (current, unitOfWork, lanes) {
			...
			try {
				...
				if ( originalError !== null && typeof originalError === 'object' && typeof originalError.then ===
						'function' ) {
	...
	function warnAboutRenderPhaseUpdatesInDEV (fiber) {
		{
			if ( isRendering && (executionContext & RenderContext) !== NoContext &&
				...
				switch (fiber.tag) {
					...
					case ForwardRef:
					case SimpleMemoComponent: {
						var renderingComponentName = workInProgress && getComponentName (workInProgress.type) ||
						...
						if ( !didWarnAboutUpdateInRenderForAnotherComponent.has (dedupeKey) ) {
							...
							error ('Cannot update a component (`%s`) while rendering a ' +
	...
	function warnIfNotCurrentlyActingEffectsInDEV (fiber) {
		{
			if ( (fiber.mode & StrictMode) !== NoMode && IsSomeRendererActing.current === false &&
				...
				error ('An update to %s ran an effect, but was not wrapped in act(...).\n\n' +
						...
						' Learn more at https://reactjs.org/link/wrap-tests-with-act', getComponentName (fiber.type))
			}
		}
	}
	...
	function warnIfNotCurrentlyActingUpdatesInDEV (fiber) {
		{
			if ( executionContext === NoContext && IsSomeRendererActing.current === false &&
					IsThisRendererActing.current === false ) {
				var previousFiber = current
	...
	function warnIfUnmockedScheduler (fiber) {
		{
			if ( didWarnAboutUnmockedScheduler === false && unstable_flushAllWithoutAsserting === undefined ) {
				if ( fiber.mode & BlockingMode || fiber.mode & ConcurrentMode ) {
					...
					error ('In Concurrent or Sync modes, the "scheduler" module needs to be mocked ' +
				...
				if ( !didWarnAboutFindNodeInStrictMode[componentName] ) {
					...
					try {
						...
						if ( fiber.mode & StrictMode ) {
							error ('%s is deprecated in StrictMode. ' +
									'%s was passed an instance of %s which is inside StrictMode. ' +
									'Instead, add a ref directly to the element you want to reference. ' +
									'Learn more about using refs safely here: ' +
									'https://reactjs.org/link/strict-mode-find-node', methodName, methodName,
									componentName)
						} else {
							error ('%s is deprecated in StrictMode. ' +
	...
	ReactDOMRoot.prototype.render = ReactDOMBlockingRoot.prototype.render = function (children) {
		...
		
		{
			if ( typeof arguments[1] === 'function' ) {
				error ('render(...): does not support the second callback argument. ' +
	...
	ReactDOMRoot.prototype.unmount = ReactDOMBlockingRoot.prototype.unmount = function () {
		{
			if ( typeof arguments[0] === 'function' ) {
				error ('unmount(...): does not support a callback argument. ' +
						'To execute a side effect after rendering, declare it in a component body with useEffect().')
			}
		}
	...
	function createRootImpl (container, tag, options) {
		...
		var mutableSources = options != null && options.hydrationOptions != null &&
	...
	function isValidContainer (node) {
		return !!(node && (node.nodeType === ELEMENT_NODE || node.nodeType === DOCUMENT_NODE || node.nodeType ===
				DOCUMENT_FRAGMENT_NODE || node.nodeType === COMMENT_NODE && node.nodeValue ===
	...
	
	{
		topLevelUpdateWarnings = function (container) {
			...
			if ( container.nodeType === ELEMENT_NODE && container.tagName && container.tagName.toUpperCase () ===
					'BODY' ) {
	...
	function legacyCreateRootFromDOMContainer (container, forceHydrate) {
			...
			while ( rootSibling = container.lastChild ) {
				{
					if ( !warned && rootSibling.nodeType === ELEMENT_NODE &&
						...
						error ('render(): Target node has markup rendered by React, but there ' +
		...
		
		{
			if ( shouldHydrate && !forceHydrate && !warnedAboutHydrateAPI ) {
				...
				warn ('render(): Calling ReactDOM.render() to hydrate server-rendered markup ' +
	...
	function findDOMNode (componentOrElement) {
		{
			...
			if ( owner !== null && owner.stateNode !== null ) {
				...
				if ( !warnedAboutRefsInRender ) {
					error ('%s is accessing findDOMNode inside its render(). ' +
							'render() should be a pure function of props and state. It should ' +
							'never access something that requires stale data from the previous ' +
							'render, such as refs. Move this logic to componentDidMount and ' +
	...
	function unmountComponentAtNode (container) {
		...
		
		{
			...
			if ( isModernRoot ) {
				error ('You are calling ReactDOM.unmountComponentAtNode() on a container that was previously ' +
		...
		if ( container._reactRootContainer ) {
		...
		} else {
			{
				...
				var isContainerReactRoot = container.nodeType === ELEMENT_NODE &&
						isValidContainer (container.parentNode) && !!container.parentNode._reactRootContainer
	...
	function unstable_createPortal (children, container) {
		...
		
		{
			if ( !didWarnAboutUnstableCreatePortal ) {
				...
				warn ('The ReactDOM.unstable_createPortal() alias has been deprecated, ' +
