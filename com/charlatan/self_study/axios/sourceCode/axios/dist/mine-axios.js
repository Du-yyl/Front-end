...
//浏览器端发送 AJAX 请求实现
		...
		var isURLSameOrigin = __webpack_require__ (/*! ./../helpers/isURLSameOrigin */
...
//引入请求的默认配置
		...
		module.exports = function dispatchRequest (config) {
			...
			// 合并一切其他头信息的配置项
			config.headers = utils.merge (config.headers.common || {}, config.headers[config.method] || {},
					config.headers)
			...
			return adapter (config).then (function onAdapterResolution (response) {
				...
				if ( !isCancel (reason) ) {
					...
					if ( reason && reason.response ) {
						reason.response.data = transformData (reason.response.data, reason.response.headers,
...
//暴露函数 根据响应状态码改变 promise 的状态
		...
		module.exports = function settle (resolve, reject, response) {
			...
			if ( !validateStatus || validateStatus (response.status) ) {
				...
				reject (
						createError ('Request failed with status code ' + response.status, response.config, null,
								response.request,
								response))
			}
		}
...
//引入工具
			var utils = __webpack_require__ (/*! ./utils */ './lib/utils.js')
//引入统一化头信息的工具函数
			var normalizeHeaderName = __webpack_require__ (/*! ./helpers/normalizeHeaderName */
...
//获取默认的适配器
			function getDefaultAdapter () {
				...
				} else if ( typeof process !== 'undefined' && Object.prototype.toString.call (process) ===
						'[object process]' ) {
...
//默认的配置项
			var defaults = {
				//适配器
				adapter: getDefaultAdapter (), //请求数据转换函数
				transformRequest: [
...
//为 default 添加头信息属性  default.headers.post  default.headers.put
			...
			/* WEBPACK VAR INJECTION */
		}.call (this, __webpack_require__ (/*! (webpack)/node_modules/process/browser.js */
