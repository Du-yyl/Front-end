...
class Snowflake {
  ...
  constructor (workerId, dataCenterId, sequence) {
    ...
    this.SEQUENCE_BITS = 9n
    this.WORKER_ID_BITS = 2n
    this.DATA_CENTER_ID_BITS = 2n
  
    this.WORKER_ID_SHIFT = this.SEQUENCE_BITS
    this.DATA_CENTER_ID_SHIFT = this.SEQUENCE_BITS + this.WORKER_ID_BITS
    this.TIMESTAMP_LEFT_SHIFT = this.SEQUENCE_BITS + this.WORKER_ID_BITS +
      this.DATA_CENTER_ID_BITS
    ...
    if (this.WORKER_ID > this.MAX_WORKER_ID || this.WORKER_ID < 0) {
      throw new Error(
        'WORKER_ID 的最大值必须大于 0 且小于 MAX_WORKER_ID [' + this.MAX_WORKER_ID + ']')
    }
    if (this.DATA_CENTER_ID > this.MAX_DATA_CENTER_ID || this.DATA_CENTER_ID <
      0) {
      throw new Error('DATA_CENTER_ID 必须最大大于 0 且小于 MAX_DATA_CENTER_ID [' +
   ...
   * - 发生时钟回拨问题会抛出异常
   * @returns {bigint} 将通过或运算进行数字的组合并返回
   */
  nextId = function () {
    let timestamp = this.timestamps()
