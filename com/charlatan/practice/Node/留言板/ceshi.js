...
var visualizationCreate = function () {
  ...
  for (var k in Recorder) {
	...
	<div style="font-size:12px;width:' + waveStore.width +
        'px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">' + config.testTitle + '</div>\
	<div style="border:1px solid #ccc;"><div style="height:' + waveStore.height + 'px;width:' + waveStore.width +
        'px;" class="' + cls + '"></div></div>\
</div>')
...
Runtime.Import([
  ...
  ,
  {
    url: RootFolder + '/assets/runtime-codes/fragment.playbuffer.js',
    check: function () {return !window.DemoFragment || !DemoFragment.PlayBuffer},
  },//引入DemoFragment.PlayBuffer
])
