...
```javascript
// 例子
const imageUrl = `http://localhost:3000/music/imageUrl?id=33894312&proxy=http://121.196.226.246:84`
...
!> 由于接口做了缓存处理 ( 缓存 2 分钟 , 可在 app.js 设置 , 可能会导致登陆后获取不 到 cookie), 相同的 imageUrl 会在两分钟内只向网易服务器发一次请求 , 如果遇到不需要缓 存结果的接口 , 可在请求
imageUrl 后面加一个时间戳参数使 imageUrl 不同 , 例子 :
`/simi/playlist?id=347230&timestamp=1503019930000`

!> 如果是跨域请求 , 请在所有请求带上 `xhrFields: { withCredentials: true }` 否则 可能会因为没带上 cookie 导致 301, 具体例子可看 `public/test.html`, 例子使用
jQuery, axios 版本也类似
...
**调用例子 :** `/music/imageUrl?id=33894312` `/music/imageUrl?id=405998841,33894312`
...
可通过 `/music/imageUrl` 接口传入歌曲 id 获取具体的播放链接
...
, 可能不能直接播放 , 需要播放的话需要调用 ' 播放 mv/视频' 接口
...
**可选参数 :** `imageUrl`: mv/视频 的 地址

**接口地址 :** `/mv/imageUrl`

**调用例子 :**
`/mv/imageUrl?imageUrl=http://v4.music.126.net/20170422034915/c98eab2f5e2c85fc8de2ab3f0f8ed1c6/web/cloudmusic/MjQ3NDQ3MjUw/89a6a279dc2acfcd068b45ce72b1f560/533e4183a709699d566180ed0cd9abe9.mp4`
...
说明 : 登陆后调用此接口 , 可获取云盘数据 , 获取的数据没有对应 imageUrl, 需要再调用一 次 `/music/imageUrl` 获取 imageUrl
...
**必选参数 :** `type`: 电台类型 , 数字 , 可通过`/dj/catelist`获取 , 对应关系为 id 对应 此接口的 type, name 对应类型意义
...
说明 : 登陆后调用此接口 , 传入`rid`, 可查看对应电台的电台节目以及对应的 id, 需要 注意的是这个接口返回的 mp3Url 已经无效 , 都为 null, 但是通过调用 `/music/imageUrl` 这 个接口 ,
传入节目 id 仍然能获取到节目音频 , 如 `/music/imageUrl?id=478446370` 获取代 码时间的一个节目的音频
