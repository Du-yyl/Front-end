...
var util = {
    ...
    isSameSign: function isSameSign (num1, num2) {
        return Math.abs(num1) === num1 && Math.abs(num2) === num2 || Math.abs(num1) !== num1 && Math.abs(num2) !==
            num2
    },
...
function createCurveControlPoints (points, i) {
    ...
    function isNotMiddlePoint (points, i) {
        if (points[i - 1] && points[i + 1]) {
            return points[i].y >= Math.max(points[i - 1].y, points[i + 1].y) || points[i].y <=
                Math.min(points[i - 1].y,
                    points[
                    i + 1].y)
        } else {
            return false
...
function getTouches (touches, opts, e) {
    ...
    if (touches.clientX) {
        if (opts.rotate) {
            ...
            x = (touches.pageY - e.currentTarget.offsetTop - (opts.height / opts.pixelRatio / 2) *
                    (opts.pixelRatio - 1)) *
                opts.pixelRatio
        } else {
            x = touches.clientX * opts.pixelRatio
            y = (touches.pageY - e.currentTarget.offsetTop - (opts.height / opts.pixelRatio / 2) *
                    (opts.pixelRatio - 1)) *
                opts.pixelRatio
        }
    } else {
    ...
    if (isInExactLegendArea(currentPoints, legendData.area)) {
        ...
        for (let i = 0, len = points.length; i < len; i++) {
            ...
            for (let j = 0; j < item.length; j++) {
                ...
                if (currentPoints.x > area[0] && currentPoints.x < area[2] && currentPoints.y > area[1] &&
...
function findFunnelChartCurrentIndex (currentPoints, funnelData) {
    ...
    for (var i = 0, len = funnelData.series.length; i < len; i++) {
        ...
        if (currentPoints.x > item.funnelArea[0] && currentPoints.x < item.funnelArea[2] && currentPoints.y >
            item.funnelArea[1] && currentPoints.y < item.funnelArea[3]) {
            currentIndex = i
            break
        }
    }
...
function findMapChartCurrentIndex (currentPoints, opts) {
    ...
    var tmp = pointToCoordinate(currentPoints.y, currentPoints.x, cData.bounds, cData.scale, cData.xoffset,
        cData.yoffset)
    var poi = [tmp.x, tmp.y]
...
function calLegendData (series, opts, config, chartData) {
    ...
    if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
        ...
        for (let i = 0; i < series.length; i++) {
            let item = series[i]
            let itemWidth = shapeWidth + shapeRight + measureText(item.name || 'undefined', fontSize) +
                opts.legend.itemGap
...
function drawPointText (points, series, config, context) {
    ...
    points.forEach(function (item, index) {
        if (item !== null) {
            ...
            context.fillText(String(formatVal),
                item.x - measureText(formatVal, series.textSize || config.fontSize) / 2, item.y -
                2)
            context.closePath()
            context.stroke()
        }
...
function drawPieText (series, opts, config, context, radius, center) {
    ...
    var seriesConvert = series.map(function (item) {
        var text = item.format ? item.format(+item._proportion_.toFixed(2)) : util.toFixed(
            item._proportion_.toFixed(4) * 100) + '%'
...
function drawToolTipSplitLine (offsetX, opts, config, context) {
    ...
    if (toolTipOption.xAxisLabel) {
        ...
        let textY = endY
        context.beginPath()
        context.setFillStyle(hexToRgb(toolTipOption.labelBgColor || config.toolTipBackground,
            toolTipOption.labelBgOpacity || config.toolTipOpacity))
        context.setStrokeStyle(toolTipOption.labelBgColor || config.toolTipBackground)
        context.setLineWidth(1 * opts.pixelRatio)
        context.rect(textX - config.toolTipPadding, textY, textWidth + 2 * config.toolTipPadding,
...
function drawMarkLine (minRange, maxRange, opts, config, context) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        if (item.showLabel) {
            ...
            context.setLineWidth(1 * opts.pixelRatio)
            context.beginPath()
            context.rect(bgStartX, textY - 0.5 * config.fontSize - config.toolTipPadding, bgWidth,
                config.fontSize + 2 * config.toolTipPadding)
            context.closePath()
            context.stroke()
            context.fill()
...
function drawToolTip (textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
    ...
    context.beginPath()
    context.setFillStyle(hexToRgb(toolTipOption.bgColor || config.toolTipBackground,
                    ...
                    points.forEach(function (item, index) {
                        if (item !== null) {
                            ...
                            if (columnOption.meter.border > 0) {
                                ...
                                context.lineTo(startX + columnOption.meter.border * 0.5,
                                    item.y + columnOption.meter.border * 0.5)
                                context.lineTo(startX + item.width - columnOption.meter.border * 0.5,
                                    item.y + columnOption.meter.border * 0.5)
...
function drawToolTipBridge (opts, config, context, process, eachSpacing, xAxisPoints) {
    var toolTipOption = opts.extra.tooltip || {}
    if (toolTipOption.horizentalLine && opts.tooltip && process === 1 &&
        (opts.type == 'line' || opts.type == 'area' || opts.type == 'column' || opts.type == 'candle' ||
            opts.type == 'mix')) {
    ...
    if (opts.xAxis.disabled !== true) {
        ...
        if (config._xAxisTextAngle_ === 0) {
            newCategories.forEach(function (item, index) {
                ...
                context.fillText(item, xAxisPoints[index] + offset,
                    startY + xAxisFontSize + (config.xAxisHeight - xAxisFontSize) / 2)
                context.closePath()
                context.stroke()
            })
            ...
            newCategories.forEach(function (item, index) {
                ...
                var _calRotateTranslate = calRotateTranslate(xAxisPoints[index] + eachSpacing / 2,
                        startY + xAxisFontSize / 2 + 5, opts.height),
        ...
        for (let i = 0; i < itemList.length; i++) {
            ...
            switch (item.legendShape) {
                case 'line':
                    ...
                    context.fillRect(startX, startY + 0.5 * lineHeight - 2 * opts.pixelRatio, 15 * opts.pixelRatio,
                        4 * opts.pixelRatio)
                    break
                case 'triangle':
                ...
                case 'circle':
                    ...
                    context.arc(startX + 7.5 * opts.pixelRatio, startY + 0.5 * lineHeight, 5 * opts.pixelRatio, 0,
                        2 * Math.PI)
                    break
                case 'rect':
                    context.moveTo(startX, startY + 0.5 * lineHeight - 5 * opts.pixelRatio)
                    context.fillRect(startX, startY + 0.5 * lineHeight - 5 * opts.pixelRatio, 15 * opts.pixelRatio,
                        10 * opts.pixelRatio)
                    break
                default:
                    context.moveTo(startX, startY + 0.5 * lineHeight - 5 * opts.pixelRatio)
...
function drawPieDataPoints (series, opts, config, context) {
    ...
    var radius = Math.min(
        (opts.width - opts.area[1] - opts.area[3]) / 2 - config.pieChartLinePadding - config.pieChartTextPadding -
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        context.setFillStyle(eachSeries.color)
        context.moveTo(centerPosition.x, centerPosition.y)
        context.arc(centerPosition.x, centerPosition.y, eachSeries._radius_, eachSeries._start_,
...
function drawArcbarDataPoints (series, opts, config, context) {
    ...
    context.beginPath()
    if (arcbarOption.type == 'default') {
        context.arc(centerPosition.x, centerPosition.y, radius, arcbarOption.startAngle * Math.PI,
...
function drawGaugeDataPoints (categories, series, opts, config, context) {
    ...
    let childendX = -radius - gaugeOption.width * 0.5 - gaugeOption.splitLine.fixRadius +
...
function drawRadarDataPoints (series, opts, config, context) {
    ...
    var _loop = function _loop (i) {
        ...
        coordinateAngle.forEach(function (angle, index) {
            var pos = convertCoordinateOrigin(radius / config.radarGridCount * i * Math.cos(angle),
                radius / config.radarGridCount *
                ...
                while (true) {
                    ...
                    if (isSpin) {
                        ...
                        areav = [
                            ...
                            opts.width - (opts.width / 2 - opts.height / 2) - (-x + tHeight + 5 + opts.height / 2) +
                            tHeight,
...
function drawFunnelDataPoints (series, opts, config, context) {
    ...
    let radius = Math.min((opts.width - opts.area[1] - opts.area[3]) / 2 - activeWidth,
...
function drawCharts (type, opts, config, context) {
    ...
    if (opts.categories && opts.categories.length) {
        ...
        let _calCategoriesData = calCategoriesData(opts.categories, opts, config,
...
Charts.prototype.showToolTip = function (e) {
    ...
    if (this.opts.type === 'line' || this.opts.type === 'area' || this.opts.type === 'column') {
        ...
        if (index > -1) {
            ...
            if (seriesData.length !== 0) {
                var _getToolTipData = getToolTipData(seriesData, this.opts.chartData.calPoints, index,
                        this.opts.categories, option),
    ...
    if (this.opts.type === 'mix') {
        ...
        if (index > -1) {
            ...
            var seriesData = getSeriesDataItem(this.opts.series, index)
            if (seriesData.length !== 0) {
                var _getMixToolTipData = getMixToolTipData(seriesData, this.opts.chartData.calPoints, index,
                        this.opts.categories, option),
    ...
    if (this.opts.type === 'candle') {
        ...
        if (index > -1) {
            ...
            var seriesData = getSeriesDataItem(this.opts.series, index)
            if (seriesData.length !== 0) {
                var _getToolTipData = getCandleToolTipData(this.opts.series[0].data, seriesData,
