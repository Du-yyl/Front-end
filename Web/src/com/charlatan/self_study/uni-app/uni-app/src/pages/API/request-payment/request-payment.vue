    ...
    <view class="uni-padding-wrap">
      <view style="background:#FFF; padding:50rpx 0;">
        ...
        <view class="uni-h1 uni-center uni-common-mt">
          ...
          <input :value="price" class="price" maxlength="4" type="digit" @input="priceChange"/>
        </view>
      </view>
      <view class="uni-btn-v uni-common-mt">
        <!-- #ifdef MP-WEIXIN -->
        <button :loading="loading" type="primary" @click="weixinPay">微信支付</button>
        <!-- #endif -->
        <!-- #ifdef APP-PLUS -->
        <template v-if="providerList.length > 0">
          <button v-for="(item,index) in providerList" :key="index" :loading="item.loading"
                  @click="requestPayment(item,index)">{{ item.name }}支付
          </button>
        </template>
        <!-- #endif -->
      </view>
