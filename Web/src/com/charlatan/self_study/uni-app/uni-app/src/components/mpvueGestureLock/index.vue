<template>
  <view :class="{error:error}" :style="{width: containerWidth +'rpx', height:containerWidth +'rpx'}"
        class="gesture-lock"
        @touchstart.stop="onTouchStart" @touchmove.stop="onTouchMove" @touchend.stop="onTouchEnd">
    <!-- 同级 v-for 的 key 重复会有问题，需要套一层。 -->
    <!-- 9 个圆 -->
    <view>
      <view v-for="(item,i) in circleArray" :key="i" :class="{check:item.check}"
    ...
    <view>
      <!-- 已激活锁之间的线段 -->
      <view v-for="(item,i) in lineArray" :key="i"
