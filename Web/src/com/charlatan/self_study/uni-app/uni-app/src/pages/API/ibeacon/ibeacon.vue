  ...
  <view>
    ...
    <view class="uni-padding-wrap uni-common-mt">
      <view class="uni-btn-v">
        <button :disabled="isOpen" type="primary" @click="openBluetoothAdapter">打开蓝牙模块</button>
        <button :disabled="!isOpen" type="primary" @click="closeBluetoothAdapter">关闭蓝牙模块</button>
        <button :disabled="!isOpen || isStarted" :loading="isStarted" type="primary" @click="startBeaconDiscovery">
          开始搜索附近的iBeacon设备
        </button>
        <button :disabled="!isStarted" type="primary" @click="stopBeaconDiscovery">停止搜索附近的iBeacon设备</button>
      </view>
    </view>
    <scroll-view class="uni-scroll_box">
      <view v-if="isStarted || deviceList.length > 0" class="uni-title">已经发现 {{ deviceList.length }} 台设备:</view>
      <view v-for="(item, index) in deviceList" :key="item.uuid" class="uni-list-box">
        <view>
