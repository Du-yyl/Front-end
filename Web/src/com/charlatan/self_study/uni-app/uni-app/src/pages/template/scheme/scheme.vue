      ...
      <form @submit="openQQ">
        ...
        <view>
          ...
          <radio-group class="uni-flex" name="qqNumType">
            <label>
              <radio checked="" value="wpa"/>
              普通QQ号</label>
            <label>
              <radio value="crm"/>
              营销QQ号(无需加好友直接聊天)</label>
          </radio-group>
        </view>
    ...
    openMap () {
      ...
      if (plus.os.name == 'Android') {
        ...
        if (hasAmap && hasBaiduMap) {
          plus.nativeUI.actionSheet({ title: '选择地图应用', cancel: '取消', buttons: [{ title: '百度地图' }, { title: '高德地图' }] },
              function (e) {
                switch (e.index) {
                  case 1:
        ...
        plus.nativeUI.actionSheet(
            { title: '选择地图应用', cancel: '取消', buttons: [{ title: 'Apple地图' }, { title: '百度地图' }, { title: '高德地图' }] },
            function (e) {
