<template>
  <view class="mpvue-picker">
    ...
    <view :class="{'mpvue-picker-view-show':showPicker}" class="mpvue-picker-content ">
      <view catchtouchmove="true" class="mpvue-picker__hd">
        <view class="mpvue-picker__action" @click="pickerCancel">取消</view>
        <view :style="{color:themeColor}" class="mpvue-picker__action" @click="pickerConfirm">确定</view>
      </view>
      <!-- 单列 -->
      <picker-view v-if="mode==='selector' && pickerValueSingleArray.length > 0" :value="pickerValue"
                   class="mpvue-picker-view"
                   indicator-style="height: 40px;" @change="pickerChange">
        <picker-view-column>
          <view v-for="(item,index) in pickerValueSingleArray" :key="index" class="picker-item">{{ item.label }}
          </view>
        </picker-view-column>
      </picker-view>
      <!-- 时间选择器 -->
      <picker-view v-if="mode==='timeSelector'" :value="pickerValue" class="mpvue-picker-view"
                   indicator-style="height: 40px;" @change="pickerChange">
        <picker-view-column>
          <view v-for="(item,index) in pickerValueHour" :key="index" class="picker-item">{{ item.label }}</view>
        </picker-view-column>
        <picker-view-column>
          <view v-for="(item,index) in pickerValueMinute" :key="index" class="picker-item">{{ item.label }}
          </view>
        </picker-view-column>
      </picker-view>
      <!-- 多列选择 -->
      <picker-view v-if="mode==='multiSelector'" :value="pickerValue" class="mpvue-picker-view"
                   indicator-style="height: 40px;" @change="pickerChange">
        <!-- #ifdef VUE3 -->
      ...
      <!-- 二级联动 -->
      <picker-view v-if="mode==='multiLinkageSelector' && deepLength===2" :value="pickerValue" class="mpvue-picker-view"
                   indicator-style="height: 40px;" @change="pickerChangeMul">
        <picker-view-column>
          <view v-for="(item,index) in pickerValueMulTwoOne" :key="index" class="picker-item">{{ item.label }}
          </view>
        </picker-view-column>
        <picker-view-column>
          <view v-for="(item,index) in pickerValueMulTwoTwo" :key="index" class="picker-item">{{ item.label }}
          </view>
        </picker-view-column>
      </picker-view>
      <!-- 三级联动 -->
      <picker-view v-if="mode==='multiLinkageSelector' && deepLength===3" :value="pickerValue" class="mpvue-picker-view"
                   indicator-style="height: 40px;" @change="pickerChangeMul">
        <picker-view-column>
          <view v-for="(item,index) in pickerValueMulThreeOne" :key="index" class="picker-item">{{ item.label }}
          </view>
        </picker-view-column>
        <picker-view-column>
          <view v-for="(item,index) in pickerValueMulThreeTwo" :key="index" class="picker-item">{{ item.label }}
          </view>
        </picker-view-column>
        <picker-view-column>
