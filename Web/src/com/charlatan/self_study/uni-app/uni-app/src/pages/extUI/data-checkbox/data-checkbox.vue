<template>
  <view>
    ...
    <uni-section subTitle="使用multiple属性开启多选" title="多选" type="line">
      <view class="uni-px-5 uni-pb-5">
        <view class="text">多选选中：{{ JSON.stringify(checkbox1) }}</view>
        <uni-data-checkbox v-model="checkbox1" :localdata="hobby" multiple></uni-data-checkbox>
      </view>
    </uni-section>

    <uni-section subTitle="使用 min / max 设置多选的最大最小值,单选无效" title="最大最小值">
      <view class="uni-px-5 uni-pb-5">
        <view class="text">选中：{{ JSON.stringify(checkbox6) }}</view>
        <uni-data-checkbox v-model="checkbox6" :localdata="hobby" max="2" min="1" multiple></uni-data-checkbox>
      </view>
    </uni-section>

    <uni-section subTitle="使用mode=button属性使用按钮样式" title="更多样式 - button" type="line">
      <view class="uni-px-5">
        <view class="text">单选选中：{{ JSON.stringify(radio2) }}</view>
        <uni-data-checkbox v-model="radio2" :localdata="sex" mode="button"></uni-data-checkbox>
      </view>
      <view class="uni-px-5 uni-pb-5">
        <view class="text">多选选中：{{ JSON.stringify(checkbox2) }}</view>
        <uni-data-checkbox v-model="checkbox2" :localdata="hobby" mode="button" multiple></uni-data-checkbox>
      </view>
    </uni-section>

    <uni-section subTitle="使用mode=tag属性使用标签样式" title="更多样式 - tag" type="line">
      <view class="uni-px-5">
        <view class="text">单选选中：{{ JSON.stringify(radio3) }}</view>
        <uni-data-checkbox v-model="radio3" :localdata="sex" mode="tag"></uni-data-checkbox>
      </view>
      <view class="uni-px-5 uni-pb-5">
        <view class="text">多选选中：{{ JSON.stringify(checkbox3) }}</view>
        <uni-data-checkbox v-model="checkbox3" :localdata="hobby" mode="tag" multiple></uni-data-checkbox>
      </view>
    </uni-section>

    <uni-section subTitle="数据中使用 disable 属性实现单独禁用,组件使用 disable 属性实现全部禁用" title="禁用" type="line">
      <view class="uni-px-5">
        <view class="text">单选选中：{{ JSON.stringify(radio4) }}</view>
        <uni-data-checkbox v-model="radio4" :localdata="sex1" mode="button"></uni-data-checkbox>
      </view>
      <view class="uni-px-5 uni-pb-5">
        <view class="text">多选选中：{{ JSON.stringify(checkbox4) }}</view>
        <uni-data-checkbox v-model="checkbox4" :localdata="hobby2" mode="button" multiple>
        </uni-data-checkbox>
      </view>
    </uni-section>


    <uni-section subTitle="使用 selectedColor 属性修改颜色" title="自定义高亮颜色" type="line">
      <view class="uni-px-5">
        <view class="text">单选选中：{{ JSON.stringify(radio5) }}</view>
        <uni-data-checkbox v-model="radio5" :localdata="sex1" selectedColor="red"></uni-data-checkbox>
      </view>
      <view class="uni-px-5 uni-pb-5">
        <view class="text">多选选中：{{ JSON.stringify(checkbox5) }}</view>
        <uni-data-checkbox v-model="checkbox5" :localdata="hobby2" multiple selectedColor="red">
        </uni-data-checkbox>
      </view>
    </uni-section>
