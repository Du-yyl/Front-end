<template>
  <uni-shadow-root class="vant-button-index">
    <button :id="id"
            ...
            :aria-label="ariaLabel" :business-id="businessId"
            :class="'custom-class '+(utils.bem('button', [type, size, { block, round, plain, square, loading, disabled, hairline, unclickable: disabled || loading }]))+' '+(hairline ? 'van-hairline--surround' : '')"
            :lang="lang" :open-type="openType" :send-message-img="sendMessageImg"
            :send-message-path="sendMessagePath" :send-message-title="sendMessageTitle"
            :session-from="sessionFrom" :show-message-card="showMessageCard" :style="(style)+' '+(customStyle)"
            hover-class="van-button--active hover-class" @click="onClick" @contact="bindContact" @error="bindError"
      ...
      <block v-else>
        <van-icon v-if="icon" :name="icon" class="van-button__icon" custom-style="line-height: inherit;"
                  size="1.2em"></van-icon>
